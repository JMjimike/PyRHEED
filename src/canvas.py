...
class Canvas(QtWidgets.QGraphicsView):
    ...
    def __init__(self, parent, config, isDarkMode):
        ...
        self._scaleBarLine = self._scene.addLine(
            QtCore.QLineF(1, 2, 1, 2), QtGui.QPen(QtCore.Qt.GlobalColor.white, 10)
        )
    ...
    def calibrate(self, scaleFactor, scaleBarLength, fontname, fontsize):
        if self.has_photo():
            ...
            x1, y1, x2, y2 = (
                180
                + self._scaleBarText.boundingRect().width() / 2
                - self.scaleBarLength / 2,
    ...
    def adjust_fonts(self, fontname, fontsize):
        if self.has_photo():
            ...
            if self._scaleBarIsPresent:
                ...
                if x1 < 30:
                    self._scaleBarText.setPos(
                        180 + 30 - x1, self.photorect.height() - 200
                    )
                    x2 += 30 - x1
                    x1 = 30
                self._scaleBarText.setFont(QtGui.QFont(fontname, fontsize))
            ...
            if self._zoom > -self.max_zoom_factor and self._zoom < self.max_zoom_factor:
                ...
                if self._photo.isUnderMouse():
                    if self._drawingLine:
                        ...
                        self.PFRadius = np.sqrt(
                            (self.start.x() - self.end.x()) ** 2
                            + (self.start.y() - self.end.y()) ** 2
                        )
                        self.draw_arc(
                            self.start, self.PFRadius, self.width, self.span, self.tilt
                        )
        ...
        if self._photo.isUnderMouse():
            if event.button() == QtCore.Qt.MouseButton.LeftButton:
                if not self._mode == "pan":
                    position = self.mapToScene(event.position())
                    self.PHOTO_MOUSE_DOUBLE_CLICK.emit(position)
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._photo.isUnderMouse():
            ...
            self.PHOTO_MOUSE_MOVEMENT.emit(position)
    ...
    def draw_line(self, start, end, EnablePlot=True):
        ...
        self._lineItem = self._scene.addLine(
            QtCore.QLineF(start, end), QtGui.QPen(QtCore.Qt.GlobalColor.yellow, 1)
        )
        self._lineItem.show()
    ...
    def draw_rect(self, start, end, width, EnablePlot=True):
        ...
        rect.append(p3)
        rect.append(p4)
        self._rectItem = self._scene.addPolygon(
            rect, QtGui.QPen(QtCore.Qt.GlobalColor.yellow, 1)
        )
        self._rectItem.show()
    ...
    def draw_arc(self, start, radius, width, span, tilt):
        self.clear_canvas()
        arc1x0, arc1y0, arc1x1, arc1y1 = (
            start.x() - (radius + width),
            start.y() - (radius + width),
            start.x() + (radius + width),
            start.y() + (radius + width),
        )
        arc2x0, arc2y0, arc2x1, arc2y1 = (
            start.x() - (radius - width),
            start.y() - (radius - width),
            start.x() + (radius - width),
            start.y() + (radius - width),
        )
        arc3x0, arc3y0, arc3x1, arc3y1 = (
            start.x() - (radius),
            start.y() - radius,
            start.x() + (radius),
            start.y() + (radius),
        ...
        self._arcItem3 = self._scene.addEllipse(
            rect3, QtGui.QPen(QtCore.Qt.GlobalColor.yellow, 1)
        )
        self._arcItem3.setStartAngle(int((270 - span / 2 + tilt) * 16))
        self._arcItem3.setSpanAngle(int(span * 16))
        self._arcItem3.show()
        self._arcItem1 = self._scene.addEllipse(
            rect1, QtGui.QPen(QtCore.Qt.GlobalColor.yellow, 1)
        )
        self._arcItem1.setStartAngle(int((270 - span / 2 + tilt) * 16))
        self._arcItem1.setSpanAngle(int(span * 16))
        self._arcItem1.show()
        self._arcItem2 = self._scene.addEllipse(
            rect2, QtGui.QPen(QtCore.Qt.GlobalColor.yellow, 1)
        )
    ...
    def save_scene(self):
        ...
        imageFileName = QtWidgets.QFileDialog.getSaveFileName(
            None,
            "choose save file name",
            os.path.join(dirname, "pattern.jpeg"),
            "Image (*.jpeg)",
        )
        rect = self._scene.sceneRect()
        capture = QtGui.QImage(
            rect.size().toSize(), QtGui.QImage.Format.Format_ARGB32_Premultiplied
        )
