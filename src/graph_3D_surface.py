...
class Graph(QtWidgets.QWidget):
    ...
    SHOW_2D_CONTOUR_SIGNAL = QtCore.pyqtSignal(
        str, bool, float, float, float, float, int, str
    )
    ...
    def run_3D_graph(self, path):
        ...
        self.numberOfContourLevelsLabel = QtWidgets.QLabel(
            "Number of Contour Levels ({})".format(50)
        )
        ...
        self.show()
        desktopRect = self.screen().availableGeometry()
        center = desktopRect.center()
        self.move(
            int(center.x() - self.width() * 0.5), int(center.y() - self.height() * 0.5)
        )
        ...
        if not path == "":
            try:
                ...
                self.raise_error(f"Failed Import of DataArray! {str(ve)}")
                self.update_log(f"Failed Import of DataArray! {str(ve)}")
    ...
    def choose_graph(self):
        path = QtWidgets.QFileDialog.getOpenFileName(
            None, "choose the graph", self.graphPath
        )
    ...
    def refresh_level_min(self):
        self.levelMinLabel.setText(
            "Level Min ({})".format(self.levelMinSlider.value() / 100)
        )
    ...
    def refresh_level_max(self):
        self.levelMaxLabel.setText(
            "Level Max ({})".format(self.levelMaxSlider.value() / 100)
        )
        if self.levelMinSlider.value() > self.levelMaxSlider.value():
    ...
    def refresh_number_of_contour_levels(self):
        self.numberOfContourLevelsLabel.setText(
            "Number of Contour Levels ({})".format(
    ...
    def show_2D_contour_button_pressed(self):
        ...
        QtCore.QCoreApplication.processEvents()
        self.SHOW_2D_CONTOUR_SIGNAL.emit(
            self.graphPath,
            True,
            self.levelMinSlider.value() / 100,
            self.levelMaxSlider.value() / 100,
    ...
    def convert_to_RTI(self, path):
        ...
        return (
            table.index.tolist(),
            [a / 180 * math.pi for a in table.columns.tolist()],
            table,
        )
...
class SurfaceGraph(QtDataVisualization.Q3DSurface):
    ...
    def __init__(self):
        ...
        self.twoDimensionalMappingSeries = QtDataVisualization.QSurface3DSeries(
            self.twoDimensionalMappingProxy
        )
    ...
    def convert_to_data_array(self, path):
        ...
        for i in range(table.shape[0]):
            ...
            for j in range(table.shape[1]):
                ...
                X, Y, Z = (
                    radius[i] * np.cos(theta[j] / 180 * math.pi),
    ...
    def enable_two_dimensional_mapping_model(self, enable):
        if enable:
            ...
            self.twoDimensionalMappingSeries.setDrawMode(
                QtDataVisualization.QSurface3DSeries.DrawFlag.DrawSurface
            )
    ...
    def set_black_to_yellow_gradient(self):
        ...
        self.seriesList()[0].setColorStyle(
            QtDataVisualization.Q3DTheme.ColorStyle.ColorStyleRangeGradient
        )
