...
from process import FitFunctions, Image
...
class Window(QtCore.QObject):
    ...
    def get_input(self):
        ...
        return dialog.getItems(
            {
                "type": "ComboBox",
                "label": "Choose the Number of Peaks",
                "content": items,
            },
            {
                "type": "ComboBox",
                "label": "Choose the Fit Function",
                "content": [
                    "gaussian",
                    "voigt",
            ...
            {
                "type": "ComboBox",
                "label": "Choose Whether to Remove a Linear Background",
                "content": ["Yes", "No"],
            },
        )
    ...
    def main(self, path, nop=1, BG=False, remove_linear_BG=False):
        ...
        if self.function == "gaussian":
            ...
            self.fit_function = (
                self.fit_worker.translational_antiphase_domain_model_intensity_using_S
            )
        ...
        for i in range(1, self.nop + 1):
            ...
            if self.function == "gaussian":
                mode = [
                    ("C", i, self.RC[-1] / self.nop * i, 0, self.RC[-1]),
                    ("H", i, float(self.initialGuess[1]), 0.01, 0.5),
                    ("W", i, float(self.initialGuess[2]), 0.01, 5),
                ]
            elif self.function == "voigt":
                mode = [
                    ("C", i, self.RC[-1] / self.nop * i, 0, self.RC[-1]),
                    ("A", i, float(self.initialGuess[1]), 0.01, 0.5),
                    ("FL", i, float(self.initialGuess[2]), 0.01, 5),
                    ("FG", i, float(self.initialGuess[3]), 0.01, 5),
                ]
            elif self.function == "translational_antiphase_domain_model":
                mode = [
                    ("G", i, float(self.initialGuess[1]), 0.0001, 1),
                    ("H", i, float(self.initialGuess[2]), 0, 5),
        ...
        if self.function == "gaussian" or self.function == "voigt":
            self.OffsetSlider = VerticalLabelSlider(
                -1, 1, 100, (self.I[0] + self.I[-1]) / 2, "O", 0
            )
    ...
    def plot_results(self, x0, guess):
        ...
        for i in range(len(guess)):
            if self.function == "gaussian":
                ...
                if i == len(guess) - 1:
                    ...
                    total = np.add(total, fit0)
                    maxH = self.fit_worker.gaussian(
                        center, height, center, width, offset
                    )
                    minH1 = self.fit_worker.gaussian(
                        x0[0], height, center, width, offset
                    )
                    minH2 = self.fit_worker.gaussian(
                        x0[-1], height, center, width, offset
                    )
                else:
                    ...
                    fit = (
                        self.fit_function(
                            x0, self.lattice_constant, gamma, height, offset
                        )
                        * fit0_normalized
                    )
                    ...
                    maxH = 1
                    minH1 = self.fit_function(
                        x0[0], self.lattice_constant, gamma, height, offset
                    )
                    minH2 = self.fit_function(
                        x0[-1], self.lattice_constant, gamma, height, offset
                    )
            ...
            self.fit_chart.profileChart.axes(QtCore.Qt.Orientation.Vertical)[
                0
            ].setRange(
                min(minH1, minH2, self.minIntensity), max(maxH, self.maxIntensity)
            )
        pen = QtGui.QPen(QtCore.Qt.DotLine)
        ...
        self.fit_chart.profileChart.axes(QtCore.Qt.Orientation.Vertical)[0].setRange(
