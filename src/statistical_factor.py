...
class Window(QtWidgets.QWidget):
    SHOW_2D_CONTOUR_SIGNAL = QtCore.pyqtSignal(
        list, float, float, float, float, str, int, str
    )
    ...
    def main(self):
        ...
        self.AsymmetricRatio = LabelSlider(
            100, 500, 1, 100, "Step Atom Density Asymmetric Ratio"
        )
    ...
    def show_2D_contour_button_pressed(self):
        ...
        self.SHOW_2D_CONTOUR_SIGNAL.emit(
            self.graph.get_data_array(),
            -self.x_range.get_value(),
            self.x_range.get_value(),
            -self.z_range.get_value(),
            self.z_range.get_value(),
            self.chooseUnit.currentText(),
    ...
    def show_2D_contour(
        ...
        lev_exp = np.linspace(
            np.floor(np.log10(intensity_min.min()) - 1),
            np.ceil(np.log10(intensity_max.max()) + 1),
            2000,
        )
        levs = np.power(10, lev_exp)
        cs = ax.contourf(x, y, intensity, levs, norm=colors.LogNorm(), cmap=cm.viridis)
    ...
    def apply(self):
        ...
        QtCore.QCoreApplication.processEvents()
        self.R_value = self.graph.refresh(
            self.Eta.get_value(),
            self.Epsilon.get_value(),
            self.AsymmetricRatio.get_value(),
            float(self.latticeConstant.text()),
            self.x_range.get_value(),
            self.x_step.get_value(),
            self.z_range.get_value(),
            self.z_step.get_value(),
            self.chooseUnit.currentText(),
        )
...
class SurfaceGraph(QtDataVisualization.Q3DSurface):
            ...
            for j in range(-countZ, countZ + 1):
                ...
                if not (i == 0 and j == 0):
                    ...
                    if self.unit == "Brillouin Zone %":
                        y = self.statistical_factor_function(
                            u / 100 * (2 * np.pi / self.lc / (np.sqrt(3) / 2)),
                            v / 100 * 2 * np.pi / self.lc / (np.sqrt(3) / 2),
                            self.epsilon,
                            self.eta,
                            self.ratio,
                        )
                        ...
                        y = self.statistical_factor_function(
                            u, v, self.epsilon, self.eta, self.ratio
                        )
                else:
                    x = 0
                    y = 1
    ...
    def statistical_factor_function(self, u, v, epsilon, eta, r):
        ...
        result = (
            (1 - R**3) ** 2
            - R**2 * (1 - R) ** 2 * (3 + 2 * (np.cos(u) + np.cos(v) + np.cos(u - v)))
        ) / (
            (1 - 2 * R * np.cos(u) + R**2)
            * (1 - 2 * R * np.cos(v) + R**2)
            * (1 - 2 * R * np.cos(u - v) + R**2)
        )
        max = ((1 - R**3) ** 2 - R**2 * (1 - R) ** 2 * (3 + 2 * 3)) / (
            (1 - 2 * R + R**2) * (1 - 2 * R + R**2) * (1 - 2 * R + R**2)
        )
        return result / max
    ...
    def enable_surface_model(self, enable):
        if enable:
            ...
            self.SurfaceSeries.setDrawMode(
