...
class Window(QtWidgets.QMainWindow):
    ...
    def __init__(self, config):
        ...
        # Preference Menu
        self.defaultSettings = self.menuPreference.addAction(
            "Default Settings", self.menu_actions_preferences_default_settings
        )

        # 2D Map Menu
        self.Two_Dimensional_Mapping = self.menu2DMap.addAction(
            "Configuration", self.menu_actions_two_dimensional_mapping
        )

        self.Three_Dimensional_Graph = self.menu2DMap.addAction(
            "3D Surface", self.menu_actions_three_dimensional_graph
        )
        ...
        self.gmm = self.menuFit.addAction(
            "Gaussian Mixture Modeling", self.menu_actions_gmm
        )
        ...
        self.open = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/open.svg")), "open", self
        )
        self.saveAs = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/save as.svg")),
            "save as",
            self,
        )
        self.zoomIn = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/zoom in.svg")),
            "zoom in (Ctrl + Plus)",
            self,
        )
        self.zoomIn.setShortcut(QtGui.QKeySequence.StandardKey.ZoomIn)
        self.zoomOut = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/zoom out.svg")),
            "zoom out (Ctrl + Minus)",
            self,
        )
        self.zoomOut.setShortcut(QtGui.QKeySequence.StandardKey.ZoomOut)
        self.fitCanvas = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/fit.svg")),
            "fit in view",
            self,
        )
        self.line = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/line.svg")), "line", self
        )
        self.line.setCheckable(True)
        self.rectangle = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/rectangle.svg")),
            "rectangle",
            self,
        )
        self.rectangle.setCheckable(True)
        self.arc = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/arc.svg")), "arc", self
        )
        self.arc.setCheckable(True)
        self.pan = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/move.svg")), "pan", self
        )
        self.pan.setCheckable(True)
        self.lightMode = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/dark.svg")), "dark mode", self
        )
        self.lightMode.setCheckable(True)
        self.darkMode = QtGui.QAction(
            QtGui.QIcon(os.path.join(self.dirname, "icons/light.svg")),
            "light mode",
            self,
        )
        ...
        self.lightMode.triggered.connect(
            lambda mode: self.toggle_dark_mode(mode="light")
        )
    ...
    def get_scale_factor(self):
        self.scaleFactor = float(
            self.properties_widget.sensitivityEdit.text()
        ) / np.sqrt(float(self.properties_widget.energyEdit.text()))
    ...
    def check_sensitivity(self, sensitivity):
        if self.check_input(sensitivity, "float"):
            self.scaleFactor = float(sensitivity) / np.sqrt(
                float(self.properties_widget.energyEdit.text())
            )
            self.SCALE_FACTOR_CHANGED.emit(self.scaleFactor)
            self.CANVAS_SCALE_FACTOR_CHANGED.emit(self.scaleFactor)
    ...
    def change_brightness(self, brightness):
        self.properties_widget.brightnessLabel.setText(
            "Brightness ({})".format(brightness)
        )

    def change_black_level(self, blackLevel):
        self.properties_widget.blackLevelLabel.setText(
            "Black Level ({})".format(blackLevel)
        )
    ...
    def apply_image_adjusts(self):
        self.update_image(
            self.currentPath,
            bitDepth=16,
            enableAutoWB=self.properties_widget.autoWBCheckBox.isChecked(),
    ...
    def reset_image_adjusts(self):
        ...
        self.properties_widget.brightnessSlider.setValue(
            int(self.propertiesDefault["brightness"])
        )
        self.properties_widget.blackLevelSlider.setValue(
            int(self.propertiesDefault["blacklevel"])
        )
        self.update_image(
            self.currentPath,
            bitDepth=16,
    ...
    def change_width(self, width):
        self.properties_widget.integralHalfWidthLabel.setText(
            "Integral Half Width ({:3.2f} \u212B\u207B\u00B9)".format(
                width / self.widthSliderScale
            )
        )
        if not self.cursorInfo.widthEdit.text() == "":
            if self._mode == "rectangle" or self._mode == "arc":
                self.cursorInfo.widthEdit.setText(
                    "{:3.2f}".format(width / self.widthSliderScale)
                )
        self.update_drawing()
        for i in range(0, self.mainTab.count()):
            self.mainTab.widget(i).width = (
                width / self.widthSliderScale * self.scaleFactor
            )
    ...
    def change_radius(self, radius):
        self.properties_widget.radiusLabel.setText(
            "Radius ({:3.2f} \u212B\u207B\u00B9)".format(
                radius / self.radiusSliderScale
            )
        )
    ...
    def change_tilt_angle(self, tilt):
        self.properties_widget.tiltAngleLabel.setText(
            "Tilt Angle ({:2.1f}\u00B0)".format(tilt / self.tiltAngleSliderScale)
        )
        self.update_drawing()
        for i in range(0, self.mainTab.count()):
    ...
    def reset_profile_options(self):
        self.properties_widget.integralHalfWidthSlider.setValue(
            int(self.width * self.widthSliderScale)
        )
        self.properties_widget.chiRangeSlider.setValue(self.chiRange)
        self.properties_widget.radiusSlider.setValue(
            int(self.radius * self.radiusSliderScale)
        )
    ...
    def edit_width(self, width):
        self.properties_widget.integralHalfWidthSlider.setValue(
            int(width * self.widthSliderScale)
        )
    ...
    def get_img_path(self):
        ...
        path = fileDlg.getOpenFileName(
            filter="TIF (*.tif;*.tiff);;Nikon (*.nef;*.nrw);;Sony (*.arw;*.srf;*.sr2);;Canon (*.crw;*.cr2;*.cr3);;JPEG (*.jpg;*.jpeg;*.jpeg2000);;GIF (*.gif);;PNG (*.png);;BMP (*.bmp);;All Files (*.*)"
        )[0]
        if not path == "":
            if not (
                os.path.splitext(path)[1] in supportedRawFormats
                or os.path.splitext(path)[1] in supportedImageFormats
            ):
    ...
    def refresh_image(
        self, path, crop, bitDepth=16, enableAutoWB=False, brightness=20, blackLevel=50
    ):
        if not path == "":
            canvas_widget = self.mainTab.currentWidget()
    ...
    def open_image(
        self, path, bitDepth=16, enableAutoWB=False, brightness=20, blackLevel=50
    ):
        if not path == "":
            ...
            self.CANVAS_SCALE_FACTOR_CHANGED.emit(self.scaleFactor)
            img_array = self.load_image(
                canvas_widget, path, bitDepth, enableAutoWB, brightness, blackLevel
            )
            self.photoList.append(img_array)
            self.pathList.append(path)
    ...
    def connect_canvas(self, canvas_widget):
        ...
        canvas_widget.KEY_PRESS_WHILE_ARC.connect(
            self.cursorInfo.chi_scan_region_update
        )
        ...
        self.properties_widget.clearButton.clicked.connect(
            canvas_widget.clear_annotations
        )
    ...
    def load_image(
        ...
        qImg, img_array = self.image_worker.get_image(
            bitDepth, path, enableAutoWB, brightness, blackLevel, crop
        )
        qPixImg = QtGui.QPixmap(qImg.size())
        QtGui.QPixmap.convertFromImage(
            qPixImg, qImg, QtCore.Qt.ImageConversionFlag.MonoOnly
        )
    ...
    def update_drawing(self):
        if not self.mainTab.count() == 0:
            if self.mainTab.currentWidget().canvasObject == "rectangle":
                self.mainTab.currentWidget().draw_rect(
                    self.mainTab.currentWidget().start,
                    self.mainTab.currentWidget().end,
                    self.properties_widget.integralHalfWidthSlider.value()
                    / 100
                    * 1
                    * self.scaleFactor,
                )
            if self.mainTab.currentWidget().canvasObject == "arc":
                self.mainTab.currentWidget().draw_arc(
                    self.mainTab.currentWidget().start,
                    self.properties_widget.radiusSlider.value()
                    / self.radiusSliderScale
                    * self.scaleFactor,
                    self.properties_widget.integralHalfWidthSlider.value()
                    / self.widthSliderScale
                    * self.scaleFactor,
                    self.properties_widget.chiRangeSlider.value(),
                    self.properties_widget.tiltAngleSlider.value()
                    / self.tiltAngleSliderScale,
                )
    ...
    def toggle_dark_mode(self, mode):
        ...
        if mode == "light":
            ...
            self.lightMode.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/dark.svg"))
            )
            self.darkMode.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/light.svg"))
            )
        elif mode == "dark":
            ...
            self.open.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/open_dark.svg"))
            )
            self.saveAs.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/save as_dark.svg"))
            )
            self.zoomIn.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/zoom in_dark.svg"))
            )
            self.zoomOut.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/zoom out_dark.svg"))
            )
            self.fitCanvas.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/fit_dark.svg"))
            )
            self.line.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/line_dark.svg"))
            )
            self.rectangle.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/rectangle_dark.svg"))
            )
            self.arc.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/arc_dark.svg"))
            )
            self.pan.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/move_dark.svg"))
            )
            self.lightMode.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/dark_dark.svg"))
            )
            self.darkMode.setIcon(
                QtGui.QIcon(os.path.join(self.dirname, "icons/light_dark.svg"))
            )
    ...
    def photo_mouse_release(self, pos, start, ShiftModified):
        if self._mode == "arc":
            self.cursorInfo.endXYEdit.setText(
                "{}".format(np.round(self.mainTab.currentWidget().PFRadius, 2))
            )
        else:
            if ShiftModified:
                if pos.x() == start.x():
            ...
            self.cursorInfo.endXYEdit.setText(
                "{},{}".format(int(pos.x()), int(pos.y()))
            )
        if self.mainTab.currentWidget()._drawingArc:
            self.properties_widget.radiusSlider.setValue(
                int(
                    self.radiusSliderScale
                    * self.mainTab.currentWidget().PFRadius
                    / self.scaleFactor
                )
            )
    ...
    def photo_mouse_double_click(self, pos):
        ...
        self.cursorInfo.intensityEdit.setText(
            "{:3.2f}".format(self._img[pos.y(), pos.x()] / np.amax(np.amax(self._img)))
        )

    def key_press_event(self, event):
        if (
            event.key() == QtCore.Qt.Key_Up
            or QtCore.Qt.Key_Down
            or QtCore.Qt.Key_Left
            or QtCore.Qt.Key_Right
        ):
    ...
    def status(self):
        status = {
            "sensitivity": float(self.properties_widget.sensitivityEdit.text()),
            ...
            "brightness": self.properties_widget.brightnessSlider.value(),
            "blackLevel": self.properties_widget.blackLevelSlider.value(),
            "integralWidth": self.properties_widget.integralHalfWidthSlider.value()
            / self.properties_widget.widthSliderScale,
            "chiRange": self.properties_widget.chiRangeSlider.value(),
            "radius": self.properties_widget.radiusSlider.value()
            / self.properties_widget.radiusSliderScale,
            "tiltAngle": self.properties_widget.tiltAngleSlider.value()
            / self.properties_widget.tiltAngleSliderScale,
