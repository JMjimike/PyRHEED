...
class Window(QtCore.QObject):
    ...
    def __init__(self):
        ...
        self.fit_colors = [
            mcolors.XKCD_COLORS["xkcd:" + name] for name in primary_colors
        ]
    ...
    def main(
        self,
        path="c:/users/yux20/documents/05042018 MoS2/interpolated_2D_stack_large.csv",
    ):
        self.startIndex = "0"
        self.endIndex = "3"
        self.range = "5"
        ...
        self.drawSampleButton.setSizePolicy(
            QtWidgets.QSizePolicy.Policy.Fixed, QtWidgets.QSizePolicy.Policy.Fixed
        )
        ...
        self.plotSampleButton.setSizePolicy(
            QtWidgets.QSizePolicy.Policy.Fixed, QtWidgets.QSizePolicy.Policy.Fixed
        )
        ...
        self.numberOfFeaturesEdit.textChanged.connect(
            self.covar_prior_table_change_features
        )
        self.numberOfFeaturesEdit.textChanged.connect(
            self.mean_prior_table_change_features
        )
        ...
        self.numberOfComponentsEdit.textChanged.connect(
            self.mean_prior_table_initialize
        )
        self.numberOfComponentsEdit.textChanged.connect(
            self.covar_prior_table_initialize
        )
        ...
        self.meanPrecPriorCheck.stateChanged.connect(
            self.mean_precision_prior_check_changed
        )
        ...
        self.distributionChart.set_fonts(
        ...
        if state == 0:
            for c in range(self.mean_prior_table.columnCount()):
                if self.mean_prior_table.item(0, 2 * c):
                    self.mean_prior_table.item(0, 2 * c).setBackground(
    ...
    def mean_prior_table_change_features(self, text):
        ...
        if nfeatures > 3:
            ...
            self.mean_prior_table.horizontalHeader().setSectionResizeMode(
                QtWidgets.QHeaderView.ResizeMode.Stretch
            )
            self.mean_prior_table.horizontalHeader().setBackgroundRole(
            ...
            for j in range(int(self.numberOfFeaturesEdit.text())):
                if not self.mean_prior_table.item(i, j * 2):
                    item = QtWidgets.QTableWidgetItem(
                        "{:.2f}".format(self.default_means[i][j])
                    )
                    item.setTextAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
                    self.mean_prior_table.setItem(i, 2 * j, item)
                else:
                    self.mean_prior_table.item(i, 2 * j).setText(
                        "{:.2f}".format(self.default_means[i][j])
                    )
        ...
        for i in range(int(self.numberOfComponentsEdit.text())):
            for j in range(int(self.numberOfFeaturesEdit.text())):
                if not self.mean_prior_table.item(i, 2 * j + 1):
                    ...
                    self.mean_prior_table.item(i, 2 * j + 1).setText(
                        "{:.2f}".format(means[i, j])
                    )
    ...
    def covar_prior_table_initialize(self, text):
        ...
        if nfeatures > 3:
            ...
            for i in range(int(ncomp) + 1):
                ...
                covar_prior_table.horizontalHeader().setSectionResizeMode(
                    QtWidgets.QHeaderView.ResizeMode.Stretch
                )
                covar_prior_table.horizontalHeader().setBackgroundRole(
                    QtGui.QPalette.ColorRole.Highlight
                )
        ...
        for i in range(1, int(self.numberOfComponentsEdit.text()) + 1):
            for j in range(int(self.numberOfFeaturesEdit.text())):
                for k in range(int(self.numberOfFeaturesEdit.text())):
                    if not self.covarTab.widget(i).item(j, k):
                        item = QtWidgets.QTableWidgetItem(
                            "{:.2f}".format(covars[i - 1, j, k])
                        )
                        item.setTextAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
                        self.covarTab.widget(i).setItem(j, k, item)
                    else:
                        self.covarTab.widget(i).item(j, k).setText(
                            "{:.2f}".format(covars[i - 1, j, k])
                        )
    ...
    def choose_source(self):
        path = QtWidgets.QFileDialog.getOpenFileName(
            ...
            "c:/users/yux20/documents/05042018 MoS2/interpolated_2D_stack_large.csv",
            filter="CSV (*.csv)",
        )[0]
    ...
    def load_data(self):
        ...
        try:
            ...
            self.informationLabel.setText(
                self.grid_3d.describe(include="all")
                .applymap(lambda x: np.around(x, 3))
                .to_string()
            )
    ...
    def draw_sample(self, level=0, change_buttons=True):
        ...
        for _ in range(int(self.numberOfDrawsEdit.text())):
            if self.stopped:
                ...
                draw = rv_discrete(
                    name="custm",
                    values=(self.grid_3d[mask].index, self.grid_3d[mask]["intensity"]),
                )
    ...
    def plot_sample(self, level=0):
        ...
        QtCore.QCoreApplication.processEvents()
        self.distributionChart.add_chart(
            self.inputdata[:, 0], self.inputdata[:, 1], "scatter"
        )
    ...
    def prepare(self):
        ...
        self.estimator = My_GMM(
            ...
            degrees_of_freedom_prior=int(self.dofEdit.text())
            if self.dofEdit.text() != ""
            else None,
    ...
    def update_plots(self, n_iter, change, params):
        if n_iter > 1:
            ...
            self.costChart.add_chart(
                self.cost_series_X, self.cost_series_Y, "ELBO change"
            )
            self.distributionChart.append_to_chart(
                x=means[:, 0],
                y=means[:, 1],
                a=a,
                b=b,
        ...
        for init in range(n_init):
            ...
            for n_iter in range(1, self.max_iter + 1):
                ...
                self.UPDATE_LOG.emit(
                    "[From sklearn.mixture.BayesGM] At iteration {}, change={}".format(
                        n_iter, change
                    )
                )
                time.sleep(0.05)
            ...
            self.UPDATE_LOG.emit(
                "[From sklearn.mixture.BayesGM] Final lower bound: {}".format(
                    lower_bound
                )
            )
