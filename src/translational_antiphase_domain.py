...
class Window(QtCore.QObject):
    ...
    REFRESH_PLOT = QtCore.pyqtSignal(
    ...
    def plot_IS(self):
        ...
        if self.unit == "h":
            ...
            self.I = (
                self.fit_worker.translational_antiphase_domain_model_intensity_using_h(
                    self.h, self.gamma_slider.get_value()
                )
            )
            self.REFRESH_PLOT.emit(
                ...
                False,
                {
                    "title": "Intensity Profile From 1D Translational-antiphase Domain Model\n(gamma={:5.3f})".format(
            ...
            self.I = (
                self.fit_worker.translational_antiphase_domain_model_intensity_using_S(
                    self.S, self.lattice_constant, self.gamma_slider.get_value()
                )
            )
            self.REFRESH_PLOT.emit(
                self.S,
    ...
    def plot_HWHM(self):
        ...
        self.width = self.fit_worker.HWHM_of_translational_antiphase_domain_model(
            1, self.gamma, self.lattice_constant
        )
        ...
        file_name = QtWidgets.QFileDialog.getSaveFileName(
            ...
            os.path.join(dirname, "HWHM vs gamma.txt"),
            "TXT (*.txt)",
        )
    ...
    def plot_contour(self):
        ...
        self.Int = (
            self.fit_worker.translational_antiphase_domain_model_intensity_using_S(
                self.h_index, self.lattice_constant, self.gamma
            )
        )
        self.figure, self.ax = plt.subplots()
        self.cs = self.ax.contourf(
            self.h_index,
            self.gamma,
            np.clip(np.log(self.Int), -7, 4),
            200,
            cmap="viridis",
        )
    ...
    def plot_2D(self):
        ...
        self.x_grid = (
            self.h_index_grid + np.cos(angle / 180 * np.pi) * self.k_index_grid
        )
        self.y_grid = np.sin(angle / 180 * np.pi) * self.k_index_grid
        self.figure_2D, self.ax_2D = plt.subplots()
        self.cs_2D = self.ax_2D.contourf(
            self.x_grid,
            self.y_grid,
            np.clip(np.log10(self.Int_2D), log_min, log_max),
            200,
            cmap="viridis",
        )
        ...
        cbar.set_ticklabels(
            list("$10^{{{}}}$".format(i) for i in range(log_min, log_max + 1, 1))
        )
