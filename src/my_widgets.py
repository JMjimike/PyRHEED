...
class DoubleSlider(QtWidgets.QWidget):
    ...
    def __init__(
        self, minimum, maximum, scale, head, tail, text, unit, direction="horizontal"
    ):
        ...
        self.minLabel = QtWidgets.QLabel(
            self.text
            + "_min = {:5.2f} ".format(self.currentMin * self.scale)
            + "("
            + unit
            + ")"
        )
        ...
        self.maxLabel = QtWidgets.QLabel(
            self.text
            + "_max = {:5.2f} ".format(self.currentMax * self.scale)
            + "("
            + unit
            + ")"
        )
...
class LabelSpinBox(QtWidgets.QWidget):
    ...
    def __init__(self, min, max, initial, scale, text, unit="", index=-1):
        ...
        if 1 / self.scale >= 0.1:
            self.label = QtWidgets.QLabel(
                self.label_text + " = {:6.1f} ".format(initial) + self.unit
            )
        elif 1 / self.scale >= 0.01:
            self.label = QtWidgets.QLabel(
                self.label_text + " = {:6.2f} ".format(initial) + self.unit
            )
        elif 1 / self.scale >= 0.001:
            self.label = QtWidgets.QLabel(
                self.label_text + " = {:6.3f} ".format(initial) + self.unit
            )
        else:
            self.label = QtWidgets.QLabel(
                self.label_text + " = {:6.4f} ".format(initial) + self.unit
            )
    ...
    def set(self, min, max, initial, scale):
        ...
        self.valueSpinBox.setMinimum(min)
        self.valueSpinBox.setMaximum(max)
        self.valueSpinBox.setValue(int(round(initial * scale)))
        self.value_changed(int(round(initial * scale)))

    def reset(self):
        self.valueSpinBox.setMinimum(self.min)
        self.valueSpinBox.setMaximum(self.max)
        self.valueSpinBox.setValue(int(round(self.initial * self.scale)))
        self.value_changed(int(round(self.initial * self.scale)))

    def value_changed(self, value):
        if 1 / self.scale >= 0.1:
            self.label.setText(
                self.label_text + " = {:6.1f} ".format(value / self.scale) + self.unit
            )
        elif 1 / self.scale >= 0.01:
            self.label.setText(
                self.label_text + " = {:6.2f} ".format(value / self.scale) + self.unit
            )
        elif 1 / self.scale >= 0.001:
            self.label.setText(
                self.label_text + " = {:6.3f} ".format(value / self.scale) + self.unit
            )
        elif 1 / self.scale >= 0.0001:
            self.label.setText(
                self.label_text + " = {:6.4f} ".format(value / self.scale) + self.unit
            )
        else:
            self.label.setText(
                self.label_text + " = {:7.5f} ".format(value / self.scale) + self.unit
            )
    ...
    def set_value(self, value):
        self.valueSpinBox.setValue(int(round(value * self.scale)))
...
class LockableDoubleSlider(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        if self.unit == "":
            if 1 / self.scale >= 1:
                self.minLabel = QtWidgets.QLabel(
                    self.text + "_min = {:2.0f} ".format(self.currentMin)
                )
            elif 1 / self.scale >= 0.1:
                self.minLabel = QtWidgets.QLabel(
                    self.text + "_min = {:3.1f} ".format(self.currentMin)
                )
            elif 1 / self.scale >= 0.01:
                self.minLabel = QtWidgets.QLabel(
                    self.text + "_min = {:4.2f} ".format(self.currentMin)
                )
            elif 1 / self.scale >= 0.001:
                self.minLabel = QtWidgets.QLabel(
                    self.text + "_min = {:5.3f} ".format(self.currentMin)
                )
            elif 1 / self.scale >= 0.0001:
                self.minLabel = QtWidgets.QLabel(
                    self.text + "_min = {:6.4f} ".format(self.currentMin)
                )
        else:
            if 1 / self.scale >= 1:
                self.minLabel = QtWidgets.QLabel(
                    self.text
                    + "_min = {:2.0f} ".format(self.currentMin)
                    + "("
                    + unit
                    + ")"
                )
                ...
                self.minLabel = QtWidgets.QLabel(
                    self.text
                    + "_min = {:3.1f} ".format(self.currentMin)
                    + "("
                    + unit
                    + ")"
                )
                ...
                self.minLabel = QtWidgets.QLabel(
                    self.text
                    + "_min = {:4.2f} ".format(self.currentMin)
                    + "("
                    + unit
                    + ")"
                )
                ...
                self.minLabel = QtWidgets.QLabel(
                    self.text
                    + "_min = {:5.3f} ".format(self.currentMin)
                    + "("
                    + unit
                    + ")"
                )
                ...
                self.minLabel = QtWidgets.QLabel(
                    self.text
                    + "_min = {:6.4f} ".format(self.currentMin)
                    + "("
                    + unit
                    + ")"
                )
        ...
        if self.unit == "":
            if 1 / self.scale >= 1:
                self.maxLabel = QtWidgets.QLabel(
                    self.text + "_max = {:2.0f} ".format(self.currentMax)
                )
            elif 1 / self.scale >= 0.1:
                self.maxLabel = QtWidgets.QLabel(
                    self.text + "_max = {:3.1f} ".format(self.currentMax)
                )
            elif 1 / self.scale >= 0.01:
                self.maxLabel = QtWidgets.QLabel(
                    self.text + "_max = {:4.2f} ".format(self.currentMax)
                )
            elif 1 / self.scale >= 0.001:
                self.maxLabel = QtWidgets.QLabel(
                    self.text + "_max = {:5.3f} ".format(self.currentMax)
                )
            elif 1 / self.scale >= 0.0001:
                self.maxLabel = QtWidgets.QLabel(
                    self.text + "_max = {:6.4f} ".format(self.currentMax)
                )
        else:
            if 1 / self.scale >= 1:
                self.maxLabel = QtWidgets.QLabel(
                    self.text
                    + "_max = {:2.0f} ".format(self.currentMax)
                    + "("
                    + unit
                    + ")"
                )
                ...
                self.maxLabel = QtWidgets.QLabel(
                    self.text
                    + "_max = {:3.1f} ".format(self.currentMax)
                    + "("
                    + unit
                    + ")"
                )
                ...
                self.maxLabel = QtWidgets.QLabel(
                    self.text
                    + "_max = {:4.2f} ".format(self.currentMax)
                    + "("
                    + unit
                    + ")"
                )
                ...
                self.maxLabel = QtWidgets.QLabel(
                    self.text
                    + "_max = {:5.3f} ".format(self.currentMax)
                    + "("
                    + unit
                    + ")"
                )
                ...
                self.maxLabel = QtWidgets.QLabel(
                    self.text
                    + "_max = {:6.4f} ".format(self.currentMax)
                    + "("
                    + unit
                    + ")"
                )
    ...
    def set_bounds(self, min_phys, max_phys):
        min_scaled = int(np.floor(min_phys * self.scale))
        max_scaled = int(np.ceil(max_phys * self.scale))
        self._minimum = min_scaled
        self._maximum = max_scaled
        if self.lock:
            self.minSlider.setMinimum(0)
            self.minSlider.setMaximum(0)
            self.maxSlider.setMinimum(0)
            self.maxSlider.setMaximum(max_scaled)
        else:
            for widget in (self.minSlider, self.maxSlider):
                widget.setMinimum(min_scaled)
                widget.setMaximum(max_scaled)

    def set_range_silent(self, head, tail):
        self.minSlider.blockSignals(True)
        self.maxSlider.blockSignals(True)
        self.currentMin = head
        self.currentMax = tail
        if not self.lock:
            self.minSlider.setValue(int(np.round(head * self.scale, 0)))
            self.maxSlider.setValue(int(np.round(tail * self.scale, 0)))
        self.minSlider.blockSignals(False)
        self.maxSlider.blockSignals(False)
        self._refresh_labels()

    def _refresh_labels(self):
        for label, value, suffix in (
            (self.minLabel, self.currentMin, "_min"),
            (self.maxLabel, self.currentMax, "_max"),
        ):
            if self.unit == "":
                if 1 / self.scale >= 1:
                    text = self.text + suffix + " = {:2.0f} ".format(value)
                elif 1 / self.scale >= 0.1:
                    text = self.text + suffix + " = {:3.1f} ".format(value)
                elif 1 / self.scale >= 0.01:
                    text = self.text + suffix + " = {:4.2f} ".format(value)
                elif 1 / self.scale >= 0.001:
                    text = self.text + suffix + " = {:5.3f} ".format(value)
                else:
                    text = self.text + suffix + " = {:6.4f} ".format(value)
            else:
                if 1 / self.scale >= 1:
                    fmt = "{:2.0f}"
                elif 1 / self.scale >= 0.1:
    ...
    def set_head(self, value):
        if not self.lock:
            self.minSlider.setValue(int(np.round(value * self.scale, 0)))

    def set_tail(self, value):
        if not self.lock:
            self.maxSlider.setValue(int(np.round(value * self.scale, 0)))
    ...
    def min_changed(self):
        ...
        self.VALUE_CHANGED.emit(
            np.round(self.currentMin, 2), np.round(self.currentMax, 2), self.index
        )
    ...
    def set_locked(self, state):
        ...
        if self.lock:
            ...
            self.maxSlider.setMinimum(int(self._minimum))
            self.minSlider.setMaximum(int(self._maximum))
...
class InfoBoard(QtWidgets.QGroupBox):
    ...
    def update(self, index, formula, a, b, c, alpha, beta, gamma):
        """This is an overload function"""
        if index == self.index:
            self.lattice_constants_label.setText(
                "  Formula: "
                + formula
                + "\n  a = {:5.3f}, b = {:5.3f}, c = {:5.3f}\n  alpha = {:5.3f}(\u00B0), beta = {:5.3f}(\u00B0), gamma = {:5.3f}(\u00B0)".format(
...
class MplCanvas(FigureCanvas):
    def __init__(
        self, parent=None, width=15, height=12, dpi=400, pos=111, facecolor="white"
    ):
        self.fig = Figure(
            figsize=(width, height), dpi=dpi, tight_layout=True, facecolor=facecolor
        )
        self.pos = pos
...
class DynamicalColorMap(QtWidgets.QWidget):
    ...
    @staticmethod
    def _real_intensity(arr):
        return np.asarray(np.abs(np.real(arr)), dtype=float)

    @staticmethod
    def _normalize_matrix(matrix):
        max_intensity = np.amax(matrix)
        if max_intensity == 0:
            max_intensity = 1.0
        return matrix / max_intensity, max_intensity
    ...
    def __init__(
        ...
        self.y_linear = y
        self.z_linear = z
        self.intensity = self._real_intensity(intensity)
        self.nkz = nkz
        ...
        if 1 in self.intensity.shape[0:1]:
            self.plot_IV = True
            self.TwoDimMappingWindow.setWindowTitle(
                self.kwargs.get("window_title", "Simulated IV curve")
            )
        elif self.type == "XZ_proj":
            self.plot_IV = False
            self.TwoDimMappingWindow.setWindowTitle("Ewald XZ projection")
        else:
            self.plot_IV = False
            self.TwoDimMappingWindow.setWindowTitle(
                self.kwargs.get(
                    "window_title", "Simulated 2D reciprocal space map"
                )
            )
        ...
        self.TwoDimMappingWindowLayout.addWidget(
            self.figure, 0, 0, QtCore.Qt.AlignmentFlag.AlignHCenter
        )
        self.TwoDimMappingWindowLayout.addWidget(
            self.toolbar, 1, 0, QtCore.Qt.AlignmentFlag.AlignHCenter
        )
        self.TwoDimMappingWindow.setWindowModality(
            QtCore.Qt.WindowModality.NonModal
            if self.kwargs.get("window_modality") == "non_modal"
        ...
        if not self.plot_IV:
            if self.type == "XY":
                if self.showFWHM:
                    ...
                    self.figureText = self.figure.axes.text(
                        self.min_x * 0.96,
                        self.max_y * 0.8,
                        "Average HWHM = {:5.4f} \u212B\u207B\u00B9\nHWHM Asymmetric Ratio = {:5.3f}".format(
                            self.HWHM, self.ratio
                        ),
                        color="white",
                ...
                self.figure.axes.set_xlabel(
                    self.kwargs.get(
                        "spectrum_x_label", r"${\bf k}_{x}$ $(\AA^{-1})$"
                    ),
                    font_dict,
                )
                self.figure.axes.set_ylabel(
                    self.kwargs.get(
                        "spectrum_y_label", r"${\bf k}_{y}$ $(\AA^{-1})$"
                    ),
                    font_dict,
                )
                title = self.kwargs.get("spectrum_title")
                if title:
                    self.figure.axes.set_title(title, fontsize=self.fontsize, pad=20)
            elif self.type == "XZ":
                title_suffix = self.kwargs.get("title_suffix", "")
                self.figure.axes.set_title(
                    "Simulated 2D reciprocal space map{}\nKy = {:5.2f} (\u212B\u207B\u00B9)".format(
                        title_suffix,
                        self.y_linear[self.nkz],
                    ),
                ...
                title_suffix = self.kwargs.get("title_suffix", "")
                self.figure.axes.set_title(
                    "Ewald XZ projection (all Ky){}".format(title_suffix),
                    fontsize=self.fontsize,
                    pad=30,
                )
                self.figure.axes.set_xlabel(r"${\bf k}_{x}$ $(\AA^{-1})$", font_dict)
            ...
            self.figure.axes.tick_params(
                width=0.1, length=1, which="both", labelsize=self.fontsize
            )
            if self.log_scale:
                self.cbar.ax.set_ylabel("Log Intensity", font_dict)
                self.cbar.set_ticks(
                    np.linspace(
                        self.log_min, self.log_max, self.log_max - self.log_min + 1
                    )
                )
                self.cbar.set_ticklabels(
                    list(
                        "$10^{{{}}}$".format(i)
                        for i in range(self.log_min, self.log_max + 1, 1)
                    )
                )
            else:
            ...
            self.figure.axes.set_xlabel(
                self.kwargs.get("spectrum_x_label", r"$K_{\perp}$ $(\AA^{-1})$"),
                font_dict,
            )
            y_default = (
                "Log Intensity\n(arb. units)"
            ...
            self.figure.axes.set_ylabel(
                self.kwargs.get("spectrum_y_label", y_default),
                font_dict,
            )
            title = self.kwargs.get("spectrum_title")
            if title:
                self.figure.axes.set_title(title, fontsize=self.fontsize, pad=15)
            self.figure.axes.tick_params(
                width=0.1, length=1, which="both", labelsize=self.fontsize
            )
            if self.pos == 211:
    ...
    def refresh_FWHM(self, showFWHM):
        ...
        if self.type == "XY":
            ...
            if not self.plot_IV:
                if self.showFWHM:
                    matrix = self._real_intensity(self.intensity[:, :, self.nkz])
                    normalized, max_intensity = self._normalize_matrix(matrix)
                    self.min_x = np.amin(self.x_linear)
                    self.max_y = np.amax(self.y_linear)
                    self.csHM = self.figure.axes.contour(
                        self.x_linear,
                        self.y_linear,
                        normalized,
                        levels=[0.5],
                    ...
                    self.FWHM = 1.0
                    self.ratio = 1.0
                    for path in self.csHM.get_paths():
                        x0 = path.vertices[:, 0]
                        y0 = path.vertices[:, 1]
        ...
        if self.type == "XY":
            ...
            if self.log_scale:
                ...
                if self.plot_IV:
                    ...
                    QtCore.QCoreApplication.processEvents()
                    self.IV = self.figure.axes.plot(
                        self.z_linear,
                        np.log10(normalized[0, :]),
                        "r-",
                        linewidth=self.IV_line_width,
                    )
                    if self.pos == 211:
                        ...
                        self.FFT = [
                            np.fft.rfftfreq(
                                length,
                                (self.z_linear[1] - self.z_linear[0]) / 2 / np.pi,
                            )[0:1000],
                            abs(np.fft.rfft(normalized[0, :], length))[0:1000],
                        ]
                        self.fft_axes.plot(
                            self.FFT[0], self.FFT[1], "b-", linewidth=self.IV_line_width
                        )
                else:
                    self.cs = self.figure.axes.contourf(
                        self.x_linear,
    ...
    def save_FFT(self, path):
        try:
            ...
            results = "\n".join(
                str(self.FFT[0][i]) + "\t" + str(self.FFT[1][i]) for i in range(1000)
            )
