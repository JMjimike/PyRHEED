...
from my_widgets import ColorPicker, LabelLineEdit, LabelMultipleLineEdit
...
class Window(QtCore.QObject):
    ...
    def main(self):
        ...
        self.lattice_constant_alpha = LabelLineEdit(
            "alpha", self.label_size, "", 2, "\u00B0"
        )
        self.lattice_constant_beta = LabelLineEdit(
            "beta", self.label_size, "", 2, "\u00B0"
        )
        self.lattice_constant_gamma = LabelLineEdit(
            "gamma", self.label_size, "", 2, "\u00B0"
        )
        ...
        self.zone_axis = LabelMultipleLineEdit(
            3, "Zone axis (h k l)", self.label_size, ["1", "1", "0"]
        )
        self.out_of_plane_orientation = LabelMultipleLineEdit(
            3, "Out-of-plane axis (h k l)", self.label_size, ["0", "0", "1"]
        )
        self.electron_energy = LabelLineEdit(
            "Electron energy", self.label_size, "15", 1, "keV"
        )
        self.incident_angle = LabelLineEdit(
            "Incident angle", self.label_size, "3", 1, "\u00B0"
        )
        self.inner_potential = LabelLineEdit(
            "Inner potential", self.label_size, "16.9", 1, "eV"
        )
        ...
        self.plot_range_hor = LabelLineEdit("Plot range k_x", self.label_size, "12", 1)
        self.plot_range_ver = LabelLineEdit("Plot range k_y", self.label_size, "12", 1)
        ...
        self.options_grid.addWidget(self.plot_range_hor, 1, 0, 1, 4)
        self.options_grid.addWidget(self.plot_range_ver, 2, 0, 1, 4)
        self.options_grid.addWidget(self.reciprocal_spot_color, 3, 0, 1, 4)
        self.options_grid.addWidget(self.kikuchi_line_color, 4, 0, 1, 4)
        self.options_grid.addWidget(self.kikuchi_envelope_color, 5, 0, 1, 4)
        self.options_grid.addWidget(self.laue_spot_color, 6, 0, 1, 4)
        self.options_grid.addWidget(self.fontListLabel, 7, 0, 1, 2)
        self.options_grid.addWidget(self.fontList, 7, 2, 1, 2)
        self.options_grid.addWidget(self.fontSizeLabel, 8, 0, 1, 2)
        self.options_grid.addWidget(self.fontSizeSlider, 8, 2, 1, 2)
        self.options_grid.addWidget(self.show_axes_label, 9, 0, 1, 1)
        self.options_grid.addWidget(self.show_axes, 9, 1, 1, 1)
        self.options_grid.addWidget(self.show_grid_label, 9, 2, 1, 1)
        self.options_grid.addWidget(self.show_grid, 9, 3, 1, 1)
        ...
        self.plot.profileChart.axes(QtCore.Qt.Orientation.Horizontal)[0].setRange(
            -self.plot_range_hor.value() / 2,
            self.plot_range_hor.value() / 2
            # TODO: Axes should reload, when input for k_x or k_y changes
        )
        self.plot.profileChart.axes(QtCore.Qt.Orientation.Vertical)[0].setRange(
            -self.plot_range_ver.value(), 0
        )
        ...
        # self.add_image(np.random.rand(100,100,3) * 255)

    def update_coordinate(self, pos):
        self.coordinate.setText(
            "(x={:5.2f} \u212B\u207B\u00B9, y={:5.2f} \u212B\u207B\u00B9)".format(
                pos.x(), pos.y()
            )
        )
    ...
    def prepare(self):
        ...
        self.G_star = self.diffraction_worker.G_star(
            self.lattice_constant_a.value(),
            self.lattice_constant_b.value(),
            self.lattice_constant_c.value(),
            self.lattice_constant_alpha.value(),
            self.lattice_constant_beta.value(),
            self.lattice_constant_gamma.value(),
        )
        self.conversion_matrix = self.diffraction_worker.conversion_matrix(
            self.lattice_constant_a.value(),
            self.lattice_constant_b.value(),
            self.lattice_constant_c.value(),
            self.lattice_constant_alpha.value(),
        ...
        self.star = [
            np.matmul(self.G_star, [1, 0, 0]),
            np.matmul(self.G_star, [0, 1, 0]),
            np.matmul(self.G_star, [0, 0, 1]),
        ]
        self.kikuchi_worker = Simulation(
            self.zone_axis.values(),
            self.out_of_plane_orientation.values(),
            self.structure,
            self.star,
            self.conversion_matrix,
            self.electron_energy.value(),
            self.incident_angle.value(),
            self.inner_potential.value(),
            int(self.index_max.value()),
            self.plot_range_hor.value(),
            self.plot_range_ver.value(),
            self.reciprocal_spot_color.get_color(),
            self.kikuchi_line_color.get_color(),
            self.kikuchi_envelope_color.get_color(),
    ...
    def open_cif(self):
        path = QtWidgets.QFileDialog.getOpenFileName(
            None, "Choose The CIF File", "./", filter="CIF (*.cif);;All Files (*.*)"
        )[0]
        if not path == "":
            self.update_log("CIF opened!")
            self.cifPath = path
            self.chooseCifLabel.setText("The path of the CIF file is:\n" + self.cifPath)
            self.structure = load_ordered_structure(self.cifPath)
    ...
    def add_line(
        ...
        series.setPen(pen)
        for x, y in zip(
            np.linspace(x1, x2, number_of_steps), np.linspace(y1, y2, number_of_steps)
        ):
            series.append(x, y)
        ...
        self.plot.profileChart.axes(QtCore.Qt.Orientation.Vertical)[0].setRange(
            0, self.plot_range_ver.value()
        )

    def add_curve(
        self, list, list_x, list_y, color="red", width=2, label="kikuchi_line"
    ):
        pen = QtGui.QPen(QtCore.Qt.PenStyle.SolidLine)
        pen.setColor(QtGui.QColor(color))
        pen.setWidth(width)
    ...
    def add_spot(
        self, list, x, y, shape="circle", color="red", size=15, label="reciprocal_spot"
    ):
    ...
    def add_image(self, image: np.ndarray):
        im_np = np.transpose(image, (1, 0, 2)).copy()
        image_qt = QtGui.QImage(
            im_np, im_np.shape[1], im_np.shape[0],                                                                                                                                                 
            QtGui.QImage.Format.Format_RGB888
        )
        brush = QtGui.QBrush(image_qt)
        self.plot.profileChart.setPlotAreaBackgroundBrush(brush)
        self.plot.profileChart.setPlotAreaBackgroundVisible(True)
...
class Simulation(QtCore.QObject):
    ...
    def __init__(
        ...
        plot_range_hor,
        plot_range_ver,
        reciprocal_spot_color,
        kikuchi_line_color,
        kikuchi_envelope_color,
        laue_spot_color,
    ):
        ...
        (
            ...
            self.plot_range_hor,
            self.plot_range_ver,
            self.reciprocal_spot_color,
            self.kikuchi_line_color,
            self.kikuchi_envelope_color,
            self.laue_spot_color,
        ) = (
            ZA,
            OPA,
            structure,
        ...
        self.HA = np.cross(
            np.matmul(self.conversion, self.ZA), np.matmul(self.conversion, self.OPA)
        )
        self._abort = False
        self.space_group_number = structure.get_space_group_info()[1]

    def run(self):
        Bin, width = self.plot_range_hor, self.plot_range_ver
        R0 = self.K_in * np.sin(self.theta_in / 180 * np.pi)
        theta_f1 = np.arccos(
            (self.K_in * np.cos(self.theta_in / 180 * np.pi) - Bin) / self.K_in
        )
        R1 = self.K_in * np.cos(self.theta_in / 180 * np.pi) * np.tan(theta_f1)
        theta_f2 = np.arccos(
            (self.K_in * np.cos(self.theta_in / 180 * np.pi) - 2 * Bin) / self.K_in
        )
        ...
        for h, k, l in itertools.product(
            range(-self.index_max, self.index_max + 1), repeat=3
        ):
            if self.diffraction_worker.is_permitted(h, k, l, self.space_group_number):
                ccc = np.transpose(
                    np.linalg.multi_dot([self.conversion, self.star, [[h], [k], [l]]])
                )
                kb = np.linalg.multi_dot(
                    [ccc, self.conversion, self.ZA]
                ) / np.linalg.norm(np.matmul(self.conversion, self.ZA))
                if abs(kb) < Bin and kb > 0.01:
                    Bin = abs(kb)
                if np.matmul(self.ZA, [[h], [k], [l]]) == 0:
                    kh = np.matmul(ccc, self.HA) / np.linalg.norm(self.HA)
                    kp = np.linalg.multi_dot(
                        [ccc, self.conversion, self.OPA]
                    ) / np.linalg.norm(np.matmul(self.conversion, self.OPA))

                    if abs(kh) < width and abs(kh) > 0.01:
                        width = abs(kh)

                    if scimath.sqrt(kh**2 + kp**2) < R1 or (
                        scimath.sqrt(kh**2 + kp**2) < 2 * R2
                        and scimath.sqrt(kh**2 + kp**2) > 2 * R1 - 1
                    ):
                        if (
                            kh > -self.plot_range_ver
                            and kh < self.plot_range_ver
                            and kp > -1e-3
                            and kp < self.plot_range_ver
                        ):
                            self.ADD_SPOT.emit(
                                (h, k, l),
                                kh,
                            ...
                            if kh / kp > 1e-6:
                                x = np.linspace(
                                    ...
                                    * (
                                        kp / 2
                                        - np.real(
                                            scimath.sqrt(0.265 * self.inner_potential)
                                        )
                                    )
                                    + kh / 2,
                                    200,
                                ...
                                x = np.linspace(
                                    -self.plot_range_ver, self.plot_range_ver, 200
                                )
                            if (
                                kp / 2 > scimath.sqrt(0.265 * self.inner_potential)
                                or kh != 0
                            ):
                                y = np.real(
                                    scimath.sqrt(
                                        (-kh / kp * (x - kh / 2) + kp / 2) ** 2
                                        - 0.265 * self.inner_potential
                                    )
                                )
                                self.ADD_CURVE.emit(
                                    (h, k, l),
            ...
            self.PROGRESS_ADVANCE.emit(
                ...
                100,
                (
                    (h + self.index_max) * (2 * self.index_max) ** 2
                    + (k + self.index_max) * (2 * self.index_max)
                    + l
                    + self.index_max
                )
                / (2 * self.index_max) ** 3
        ...
        if not self._abort:
            ...
            for i in range(-self.index_max, self.index_max + 1):
                ...
                xr1 = (
                    (i + 1 / 2)
                    * width
                    * np.cos(self.theta_in / 180 * np.pi)
                    / np.cos(theta_f1)
                )
                ...
                self.PROGRESS_ADVANCE.emit(
                    0, 100, (i + self.index_max) / (2 * self.index_max) * 100
                )
                QtCore.QCoreApplication.processEvents()
                if self._abort:
                    break
            ...
            if not self._abort:
                ...
                for m in range(-3, 4):
                    if not m == 0:
                        ...
                        if m > 0:
                            x_ke1 = np.linspace(
                                (U - Bm**2) / 2 / Bm, self.plot_range_hor, 200
                            )
                            x_ke2 = np.linspace(-Bm / 2, self.plot_range_hor, 200)
                        else:
                            x_ke1 = np.linspace(
                                -self.plot_range_hor, (U - Bm**2) / 2 / Bm, 200
                            )
                            x_ke2 = np.linspace(-self.plot_range_hor, -Bm / 2, 200)
