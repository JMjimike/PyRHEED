...
from process import FitBroadening, FitFunctions, Image
...
class Window(QtCore.QObject):
    ...
    def main(self, path):
        ...
        self.chooseDestinationLabel = QtWidgets.QLabel(
        ...
        self.destinationGrid.setAlignment(
            self.chooseDestinationButton, QtCore.Qt.AlignmentFlag.AlignRight
        )
        ...
        self.fitFunctionCombo.addItem(
            "Translational Antiphase Domain Model",
        ...
        self.numberOfPeaksCombo.currentIndexChanged.connect(
            self.number_of_peaks_changed
        )
        ...
        self.statusGrid.setAlignment(
            self.progressBar, QtCore.Qt.AlignmentFlag.AlignRight
        )
        self.ButtonBox = QtWidgets.QDialogButtonBox()
        self.ButtonBox.addButton(
            "Start", QtWidgets.QDialogButtonBox.ButtonRole.ActionRole
        )
        self.ButtonBox.addButton(
            "Stop", QtWidgets.QDialogButtonBox.ButtonRole.ActionRole
        )
        self.ButtonBox.addButton(
        ...
        self.ButtonBox.findChildren(QtWidgets.QPushButton)[1].clicked.connect(self.stop)
        self.ButtonBox.findChildren(QtWidgets.QPushButton)[2].clicked.connect(
            self.reset
        )
        self.ButtonBox.findChildren(QtWidgets.QPushButton)[3].clicked.connect(
            self.reject
        )
        ...
        self.table.horizontalHeader().setBackgroundRole(
    ...
    def prepare(self):
        if self.table_is_ready():
            ...
            if self.saveResult.checkState() == 2:
                ...
                self.output.write(
                    QtCore.QDateTime.currentDateTime().toString(
                        "MMMM d, yyyy  hh:mm:ss ap"
                    )
                    + "\n"
                )
                self.output.write(
                    "The source directory is: " + self.currentSource + "\n"
                )
            index = []
            ...
            if self.saveResult.checkState() == 2:
                ...
                if self.fitFunctionCombo.currentText() == "Gaussian":
                    results_head = (
                        ...
                        + "\t"
                        + "\t".join(
                            str(label + i).ljust(12)
                            + "\t"
                            + str(label + i + "_error").ljust(12)
                            for label in ["H", "C", "W"]
                            for i in index
                        )
                        + "\t"
            ...
            if (
                ...
                self.logBox.append(
                    QtCore.QTime.currentTime().toString("hh:mm:ss")
                    + "\u00A0\u00A0\u00A0\u00A0ERROR: Please choose the region!"
                )
                ...
                self.logBox.append(
                    QtCore.QTime.currentTime().toString("hh:mm:ss")
                    + "\u00A0\u00A0\u00A0\u00A0ERROR: Please choose the origin!"
                )
                ...
                self.broadening_worker = FitBroadening(
                    path,
                    ...
                    self.BGCheck.checkState(),
                    self.remove_linear_BGCheck.checkState(),
                    self.saveResult.checkState(),
                    self.fitFunctionCombo.currentText(),
                    self.guess,
                    (self.bound_low, self.bound_high),
                    float(self.FTolEdit.text()),
                    float(self.XTolEdit.text()),
                    float(self.GTolEdit.text()),
                    self.method.currentData(),
                    self.loss.currentData(),
                )
                ...
                self.broadening_worker.DRAW_LINE_REQUESTED.connect(
                    self.DRAW_LINE_REQUESTED
                )
                self.broadening_worker.DRAW_RECT_REQUESTED.connect(
                    self.DRAW_RECT_REQUESTED
                )
                self.broadening_worker.ADD_COST_FUNCTION.connect(
                    self.plot_cost_function
                )
                self.broadening_worker.ADD_PLOT.connect(self.plot_results)
                self.FEED_BACK_TO_FIT_WORKER.connect(
                    self.broadening_worker.update_fitting_parameters
                )
    ...
    def change_fit_function(self, function):
        if function == "Gaussian":
            ...
            self.table.setHorizontalHeaderItem(
                11, QtWidgets.QTableWidgetItem("FG_High")
            )
                ...
                for i in range(self.table.rowCount()):
                    ...
                    if j == 3:
                        item2 = QtWidgets.QTableWidgetItem(
                            "{}".format(max(0, np.round(value - variation[0], 2)))
                        )
                        item3 = QtWidgets.QTableWidgetItem(
                            "{}".format(np.round(value + variation[1], 2))
                        )
                    # Center
                    elif j == 0:
                        item2 = QtWidgets.QTableWidgetItem(
                            "{}".format(max(0, np.round(value - variation[2], 2)))
                        )
                        item3 = QtWidgets.QTableWidgetItem(
                            "{}".format(np.round(value + variation[3], 2))
                        )
                    # Width
                    else:
                        item2 = QtWidgets.QTableWidgetItem(
                            "{}".format(max(0.1, np.round(value - variation[4], 2)))
                        )
                        item3 = QtWidgets.QTableWidgetItem(
                            "{}".format(np.round(value + variation[5], 2))
                        )
                    item.setForeground(
                        QtGui.QBrush(QtGui.QColor(QtCore.Qt.GlobalColor.red))
                    )
            ...
            for j in [0, 3, 6, 9]:
                for i in range(self.table.rowCount()):
                    ...
                    # FL
                        ...
                        item2 = QtWidgets.QTableWidgetItem(
                            "{}".format(max(0.01, np.round(value - variation[4], 2)))
                        )
                        item3 = QtWidgets.QTableWidgetItem(
                            "{}".format(np.round(value + variation[5], 2))
                        )
                    # FG
                        ...
                        item2 = QtWidgets.QTableWidgetItem(
                            "{}".format(max(0.01, np.round(value - variation[6], 2)))
                        )
                        item3 = QtWidgets.QTableWidgetItem(
                            "{}".format(np.round(value + variation[7], 2))
                        )
                    item.setForeground(
                        QtGui.QBrush(QtGui.QColor(QtCore.Qt.GlobalColor.red))
                    )
    ...
    def plot_results(self, x0, y0, Kperp, nimg):
        if not self.file_has_been_created:
            if self.BGCheck.isChecked() and self.saveResult.isChecked():
                ...
                self.save_path_without_background = (
                    self.currentDestination + "/2D_map_without_background.txt"
                )
        ...
        for i in range(self.table.rowCount()):
            if self.fitFunctionCombo.currentText() == "Gaussian":
                ...
                total = np.add(total, fit0)
                maxH = self.fit_function(
                    center, center, amplitude, width_L, width_G, offset
                )
                minH1 = self.fit_function(
                    x0[0], center, amplitude, width_L, width_G, offset
                )
                minH2 = self.fit_function(
                    x0[-1], center, amplitude, width_L, width_G, offset
                )
            ...
            self.chart.profileChart.axes(QtCore.Qt.Orientation.Vertical)[0].setRange(
    ...
    def table_is_ready(self):
        ...
        for j in j_list:
            for i in range(self.table.rowCount()):
                if not (
                    float(self.table.item(i, j).text())
                    >= float(self.table.item(i, j + 1).text())
                    and float(self.table.item(i, j).text())
                    <= float(self.table.item(i, j + 2).text())
                ):
    ...
    def show_manual_fit(self):
        self.STATUS_REQUESTED.emit()
        self.ManualFitWindow = manual_fit.Window(
            self.fontList.currentFont().family(),
            self.fontSizeSlider.value(),
            self.fitFunctionCombo.currentData(),
        )
        ...
        if not image_list == []:
            self.ManualFitWindow.main(
                image_list[startIndex],
                self.table.rowCount(),
                self.BGCheck.checkState(),
                self.remove_linear_BGCheck.checkState(),
            )
