import numpy as np
from PyQt6 import QtCharts, QtCore, QtGui, QtSvg, QtWidgets
...
class ProfileChart(QtCharts.QChartView):
    ...
    def append_to_chart(self, x, y, a, b, angle, weights, colors, type="ellipse"):
        ...
        for n in range(ncomp):
            ...
            delta_a = ea * (1 - np.cos(angle[n] / 180 * np.pi)) - eb * np.sin(
                angle[n] / 180 * np.pi
            )
            delta_b = ea * np.sin(angle[n] / 180 * np.pi) + eb * (
                1 - np.cos(angle[n] / 180 * np.pi)
            )
            ...
            ellipse_painter.translate(
                msize // 2 - ea // 2 + delta_a // 2, msize // 2 - eb // 2 + delta_b // 2
            )
    ...
    def set_axes_visible(self, state):
        if state == 2:
            ...
            self.profileChart.axes(QtCore.Qt.Orientation.Horizontal)[0].setVisible(
                False
            )
    ...
    def set_grids_visible(self, state):
        if state == 2:
            self.profileChart.axes(QtCore.Qt.Orientation.Horizontal)[
                0
            ].setGridLineVisible(True)
            self.profileChart.axes(QtCore.Qt.Orientation.Vertical)[
                0
            ].setGridLineVisible(True)
        elif state == 0:
            self.profileChart.axes(QtCore.Qt.Orientation.Horizontal)[
    ...
    def integral(self, start, end, width):
        x, y = self.image_worker.get_integral(
            start, end, width, self._img, self._scaleFactor
        )
        x0, y0, x1, y1 = start.x(), start.y(), end.x(), end.y()
        slope = abs((x0 - x1) / (y1 - y0)) if y1 != y0 else 0
    ...
    def chi_scan(self, center, radius, width, chiRange, tilt, chiStep=1):
        x, y = self.image_worker.get_chi_scan(
            center, radius, width, chiRange, tilt, self._img, chiStep
        )
    ...
    def resizeEvent(self, event):
        super(ProfileChart, self).resizeEvent(event)
    ...
    def save_profile_as_text(self):
        if self.CHART_IS_PRESENT:
            self.filename = QtWidgets.QFileDialog.getSaveFileName(
                None, "choose save file name", "./profile.txt", "Text (*.txt)"
            )
            if not self.filename[0] == "":
                np.savetxt(
                    self.filename[0],
                    np.vstack((self.currentRadius, self.currentProfile)).transpose(),
                    fmt="%5.3f",
                )
            else:
                return
        ...
        if self.CHART_IS_PRESENT:
            ...
            if not self.filename[0] == "":
                ...
                output_rect = QtCore.QRectF(
                    QtCore.QPointF(0, 0), QtCore.QSizeF(output_size)
                )
    ...
    def save_profile_as_SVG(self):
        if self.CHART_IS_PRESENT:
            self.filename = QtWidgets.QFileDialog.getSaveFileName(
                None, "choose save file name", "./profile.svg", "SVG (*.svg)"
            )
            if not self.filename[0] == "":
                output_size = QtCore.QSize(800, 600)
                output_rect = QtCore.QRectF(
