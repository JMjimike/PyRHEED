    ...
    def __init__(self, *args, **kwargs):
        ...
        for j, obj in enumerate(args):
            self.table.setHorizontalHeaderItem(
                j, QtWidgets.QTableWidgetItem(obj.__class__.__name__)
            )
            total_size = 0
            for att in dir(obj):
                ...
                if size >= 1024000:
                    ...
                    item.setForeground(
                        QtGui.QBrush(QtGui.QColor(QtCore.Qt.GlobalColor.black))
                    )
                item.setTextAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
                self.table.setItem(self.row_dict[att], j, item)
            self.table.setItem(
                0, j, QtWidgets.QTableWidgetItem("{}".format(total_size))
            )
        ...
        desktopRect = self.widget.screen().availableGeometry()
        center = desktopRect.center()
        self.widget.move(
            center.x() - self.widget.width() * 0.5,
            center.y() - self.widget.height() * 0.5,
        )
            ...
            for att in dir(obj):
                if not att in self.row_dict:
                    ...
                    self.table.setVerticalHeaderItem(
                        self.row_dict[att], QtWidgets.QTableWidgetItem(att)
                    )
                    self.row_number += 1
                size = getsizeof(getattr(obj, att))
                total_size += size
