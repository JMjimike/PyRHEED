...
from process import Convertor, Image, ReciprocalSpaceMap
...
class Window(QtCore.QObject):
    ...
    def main(self, path):
        ...
        self.azimuthRangeCombo.addItem("0~90", 90)
        ...
        self.normalizationMethodCombo.currentTextChanged.connect(
            self.normalization_method_changed
        )
        ...
        self.centralisationMethodCombo.currentTextChanged.connect(
            self.centralisation_method_changed
        )
        ...
        self.chooseOffsetLabel = QtWidgets.QLabel(
            "The offset file directory is:\n" + self.currentOffset
        )
        self.chooseOffsetButton = QtWidgets.QPushButton("Browse...")
        self.chooseOffsetButton.setSizePolicy(
            QtWidgets.QSizePolicy.Policy.Fixed, QtWidgets.QSizePolicy.Policy.Fixed
        )
        ...
        self.colormap.addItem("viridis", "viridis")
        self.colormap.addItem("hsv", "hsv")
        self.colormap.addItem("turbo", "turbo")
        self.colormap.addItem("plasma", "plasma")
        self.levelMinLabel = QtWidgets.QLabel(
            "Level Min ({})".format(self.levelMin / 100)
        )
        ...
        self.levelMaxLabel = QtWidgets.QLabel(
            "Level Max ({})".format(self.levelMax / 100)
        )
        ...
        self.numberOfContourLevelsLabel = QtWidgets.QLabel(
            "Number of Levels ({})".format(self.numberOfContourLevels)
        )
        self.numberOfContourLevelsSlider = QtWidgets.QSlider(
        ...
        self.numberOfContourLevelsSlider.valueChanged.connect(
            self.refresh_number_of_contour_levels
        )
    ...
    def show_2D_contour_button_clicked(self):
        self.SHOW_2D_CONTOUR.emit(
            self.graphTextPath,
            False,
            self.levelMinSlider.value() / 100,
            self.levelMaxSlider.value() / 100 * self.normalization_factor,
            self.radiusMinSlider.value() / 100,
            self.radiusMaxSlider.value() / 100,
            self.numberOfContourLevelsSlider.value(),
    ...
    def centralisation_method_changed(self, text):
        if text == "none":
            self.azimuthRangeCombo.setCurrentText("0~90")
            self.polar.setChecked(True)
    ...
    def prepare(self):
        ...
        self.reciprocal_space_worker = ReciprocalSpaceMap(
            self.status,
            path,
            self.windowDefault,
            self.poleFigure.isChecked(),
            self.azimuthRangeCombo.currentData(),
            self.normalizationMethodCombo.currentData(),
            self.centralisationMethodCombo.currentData(),
            self.saveResults.isChecked(),
            self.twoD.isChecked(),
            self.cartesian.isChecked(),
            startIndex,
            endIndex,
            analysisRange,
            ...
            fileType,
            self.grouping.value(),
            self.synchronize.isChecked(),
            float(self.sleepTimeEdit.text()),
            self.currentOffset,
        )
        ...
        self.reciprocal_space_worker.DRAW_LINE_REQUESTED.connect(
            self.DRAW_LINE_REQUESTED
        )
        self.reciprocal_space_worker.DRAW_RECT_REQUESTED.connect(
    ...
    def choose_offset(self):
        path = QtWidgets.QFileDialog.getOpenFileName(
            None, "choose the offset file", self.currentOffset, filter="Excel (*.xlsx)"
        )[0]
        self.currentOffset = path
        self.chooseOffsetLabel.setText(
            "The offset file directory is:\n" + self.currentOffset
        )
    ...
    def refresh_radius_min(self):
        self.radiusMinLabel.setText(
            "Radius Min ({})".format(self.radiusMinSlider.value() / 100)
        )
    ...
    def refresh_radius_max(self):
        self.radiusMaxLabel.setText(
            "Radius Max ({})".format(self.radiusMaxSlider.value() / 100)
        )
        if self.radiusMinSlider.value() > self.radiusMaxSlider.value():
    ...
    def refresh_number_of_contour_levels(self):
        ...
        self.numberOfContourLevelsLabel.setText(
            "Number of Levels ({})".format(self.numberOfContourLevels)
        )
