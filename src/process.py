...
from scipy.spatial import ConvexHull, Voronoi, cKDTree, voronoi_plot_2d
from scipy.stats import rv_discrete
from shapely.geometry import LineString, MultiPoint, Point, Polygon
from shapely.ops import unary_union

from structure_io import load_ordered_structure


class Image(object):
    def __init__(self):
        super(Image, self).__init__()
        self.supportedRawFormats = {
            ".3fr",
            ".ari",
            ".arw",
            ".srf",
    ...
    def get_image(
        self, bit_depth, img_path, EnableAutoWB, Brightness, UserBlack, image_crop
    ):
    ...
    def get_line_scan(self, start, end, img, scale_factor, normalize_to_img_max=True):
        ...
        for i in range(0, len(Kx)):
            try:
                ...
                pass
        LineScanRadius = np.linspace(
            0, math.sqrt((x1 - x0) ** 2 + (y1 - y0) ** 2), len(Kx)
        )
    ...
    def get_integral(
        self, start, end, width, img, scale_factor, normalize_to_img_max=True
    ):
        x0, y0, x1, y1 = start.x(), start.y(), end.x(), end.y()
        K_length = max(int(abs(x1 - x0) + 1), int(abs(y1 - y0) + 1))
        int_width = int(width)
        ...
        if y1 == y0:
            for i in range(0, len(Kx)):
                try:
                    LineScanIntensities[i] = np.sum(
                        img[int(Ky[i]) - int_width : int(Ky[i]) + int_width, int(Kx[i])]
                    )
                except:
                    pass
        elif x1 == x0:
            for i in range(0, len(Kx)):
                try:
                    LineScanIntensities[i] = np.sum(
                        img[int(Ky[i]), int(Kx[i]) - int_width : int(Kx[i]) + int_width]
                    )
                except:
                    pass
        else:
                    ...
                    index = np.asarray(
                        [
                            [
                                ...
                                np.linspace(
                                    Kx[i] - int_width / slope,
                                    Kx[i] + (int_width + 1) / slope,
                                    2 * int_width + 1,
                                ),
                            ]
                ...
                try:
                    index = np.asarray(
                        [
                            [
                                np.linspace(
                                    Ky[i] - int_width * slope,
                                    Ky[i] + (int_width + 1) * slope,
                                    2 * int_width + 1,
                                ),
                                np.linspace(
                                    Kx[i] - int_width,
                    ...
                    pass
            try:
                LineScanIntensities = np.sum(
                    [img[index[i, 0, :], index[i, 1, :]] for i in range(len(Kx))],
                    axis=1,
                )
            except:
                pass
        normalization_factor = np.amax(np.amax(img)) if normalize_to_img_max else 1
        return (
            LineScanRadius / scale_factor,
            LineScanIntensities / 2 / width / normalization_factor,
        )
    ...
    def get_chi_scan(
        ...
        if int(chiRange / chiStep) > 2:
            ...
            ChiTotalSteps = 2
        ChiAngle = np.linspace(
            -chiRange / 2 - tilt + 90, chiRange / 2 - tilt + 90, ChiTotalSteps + 1
        )
        ...
        cit = 0
        if ChiAngle[0] <= 90.0 and ChiAngle[0 + 1] > 90.0:
            y5 = y0 + radius + width
        else:
            y5 = 0
        for i in range(
            int(np.amin([y1, y2, y3, y4])), int(np.amax([y1, y2, y3, y4, y5])) + 1
        ):
            for j in range(
                int(np.amin([x1, x2, x3, x4])), int(np.amax([x1, x2, x3, x4])) + 1
            ):
                if (
                    (j - x0) ** 2 + (i - y0) ** 2 > (radius - width) ** 2
                    and (j - x0) ** 2 + (i - y0) ** 2 < (radius + width) ** 2
                    and (j - x0) / np.sqrt((i - y0) ** 2 + (j - x0) ** 2)
                    < np.cos(ChiAngle[0] * np.pi / 180)
                    and (j - x0) / np.sqrt((i - y0) ** 2 + (j - x0) ** 2)
                    > np.cos(ChiAngle[1] * np.pi / 180)
                ):
                    indices = np.append(indices, [[i, j]], axis=0)
                    cit += 1
        RotationTensor = np.array(
            [
                [
                    [
                        -np.sin((theta - ChiAngle[0]) * np.pi / 180),
                        np.cos((theta - ChiAngle[0]) * np.pi / 180),
                    ],
                    [
                        np.cos((theta - ChiAngle[0]) * np.pi / 180),
                        np.sin((theta - ChiAngle[0]) * np.pi / 180),
        ...
        ImageIndices = np.tensordot(
            RotationTensor, (indices[1:-1] - [y0, x0]).T, axes=1
        ).astype(int)
        try:
            ChiProfile = (
                np.sum(
                    [
                        img[
                            ImageIndices[i, 1, :] + int(y0),
                            ImageIndices[i, 0, :] + int(x0),
                        ]
                        for i in range(ChiTotalSteps + 1)
                    ],
                    axis=1,
                )
                / cit
...
class FitFunctions(object):
    ...
    def translational_antiphase_domain_model_intensity_using_h(self, h, gamma):
        result = (
            1
            - np.multiply(
                self.boundary_structure_factor_using_h(h, gamma),
                self.boundary_structure_factor_using_h(h, gamma),
            )
        ) / (
            1
            ...
            - 2
            * np.multiply(
                self.boundary_structure_factor_using_h(h, gamma), np.cos(2 * np.pi * h)
            )
        )
        return np.where(np.isnan(result), 100, result)

    def translational_antiphase_domain_model_intensity_using_S(
        self, S, a, gamma, height=1, offset=0
    ):
        result = (
            1
            - np.multiply(
                self.boundary_structure_factor_using_S(S, a, gamma),
                self.boundary_structure_factor_using_S(S, a, gamma),
            )
        ) / (
            1
    ...
    def translational_antiphase_domain_model_intensity_2D(self, h, k, gamma_a, gamma_b):
        return np.multiply(
            self.translational_antiphase_domain_model_intensity_using_h(h, gamma_a),
            self.translational_antiphase_domain_model_intensity_using_h(k, gamma_b),
        )

    def translational_antiphase_domain_model_intensity_2D_four_indices(
        self, h, k, i, gamma_a, gamma_b, gamma_c
    ):
        return np.multiply(
            np.multiply(
                self.translational_antiphase_domain_model_intensity_using_h(h, gamma_a),
    ...
    def translational_antiphase_domain_model_intensity_2D_without_approximation(
        self, h, k, gamma_a, gamma_b, m1, m2
    ):
        ...
        for it1 in range(-m1, m1 + 1):
            for it2 in range(-m2, m2 + 1):
                result += np.multiply(
                    np.multiply(
                        np.power(
                            self.boundary_structure_factor_using_h(h, gamma_a), m1
                        ),
                        np.power(
                            self.boundary_structure_factor_using_h(k, gamma_b), m2
                        ),
                    ),
    ...
    def HWHM_of_translational_antiphase_domain_model(self, h, gamma, a):
        alpha = (
            4 * self.boundary_structure_factor_using_h(h, gamma)
            - np.multiply(
                self.boundary_structure_factor_using_h(h, gamma),
                self.boundary_structure_factor_using_h(h, gamma),
            )
            - 1
        ) / (2 * self.boundary_structure_factor_using_h(h, gamma))
    ...
    def boundary_structure_factor_using_S(self, S, a, gamma):
        ...
        prob = 0
        for n in range(0, 45):
            sum += np.cos((1 + n / 45) * S * a * np.cos(np.pi / 6)) * self.probability(
                n, 45
            )
    ...
    def gaussian(self, x, height, center, FWHM, offset=0):
        ...
        return (
            height
            / (FWHM * math.sqrt(math.pi / (4 * math.log(2))))
            * np.exp(-4 * math.log(2) * (x - center) ** 2 / (FWHM**2))
            + offset
        )
    ...
    def get_multiple_voigts(self, n):
        ...
        def multiple_voigts(x, *args):
            ...
            for i in range(n):
                offset += voigt(
                    x, args[i], args[i + n], args[i + 2 * n], args[i + 3 * n]
                )
            return offset
    ...
    def test(self):
        raw_3d = pandas.read_csv(
            filepath_or_buffer="c:/users/yux20/documents/05042018 MoS2/3D_Map_04162019.txt",
            sep=" ",
        ...
        rawx = raw_3d.iloc[subset, [0]].T.to_numpy() * np.cos(
            raw_3d.iloc[subset, [1]].T.to_numpy() / np.pi
        )
        rawy = raw_3d.iloc[subset, [0]].T.to_numpy() * np.sin(
            raw_3d.iloc[subset, [1]].T.to_numpy() / np.pi
        )
        ...
        interp = LinearNDInterpolator(
            list(zip(rawx[0], rawy[0], rawz[0])), intensity[0]
        )
        ...
        output = open(
            "c:/users/yux20/documents/05042018 MoS2/interpolated_3D_map.txt", mode="w"
        )
        for i in range(nx):
            for j in range(ny):
                for k in range(nz):
                    ...
                    row = (
                        "\t".join(
                            [
                                str(np.around(x[j][i][k], 4)),
                                str(np.around(y[j][i][k], 4)),
                                str(np.around(z[j][i][k], 4)),
                                str(np.around(interp_3d[j][i][k] / intensity_sum, 10)),
                            ]
                        )
                        + "\n"
                    )
...
class Convertor(object):
    ...
    def txt2vtp(self, path, type):
        ...
        filename = (
            os.path.dirname(path) + "/" + os.path.basename(path).split(".")[0] + ".vtp"
        )
    ...
    def mtx2vtp(
        ...
        for i, j, k in itertools.product(
            list(range(N_para)), list(range(N_para)), list(range(N_perp))
        ):
            ...
            data[i * N_para * N_perp + j * N_perp + k, 3] = (
                matrix[i, j, k] / max_intensity
            )
        vtkFile = ET.Element("VTKFile")
        vtkFile.set("type", "PolyData")
        vtkFile.set("version", "0.1")
        ...
        output.write("Time: \n")
        output.write(
            QtCore.QDateTime.currentDateTime().toString("MMMM d, yyyy  hh:mm:ss ap")
            + "\n\n"
        )
        ...
        output.write("\n\n")
        results = "\n".join(
            "\t".join(str(data[i, j]) for j in range(4))
            for i in range(N_para * N_para * N_perp)
        )
...
class Diffraction(object):
    ...
    def G_matrix(self, a, b, c, alpha, beta, gamma):
        return np.array(
            [
                [
                    a**2,
                    a * b * np.cos(gamma / 180 * np.pi),
                    a * c * np.cos(beta / 180 * np.pi),
                ],
                [
                    a * b * np.cos(gamma / 180 * np.pi),
                    b**2,
                    b * c * np.cos(alpha / 180 * np.pi),
                ],
                [
                    a * c * np.cos(beta / 180 * np.pi),
                    b * c * np.cos(alpha / 180 * np.pi),
    ...
    def conversion_matrix(self, a, b, c, alpha, beta, gamma):
        ...
        c2 = (
            ...
            * (
                ...
                - np.cos(gamma / 180 * np.pi) * np.cos(beta / 180 * np.pi)
            )
    ...
    def is_permitted(self, h, k, l, space_group_number):
        if space_group_number == 167:
            return ((-h + k + l) % 3 == 0 and h != -k) or (
                (h + l) % 3 == 0 and l % 2 == 0 and h == -k
            )
    ...
    def run(self):
        ...
        if mode == "arc" and self.IsPoleFigure:
            if (
                ...
                self.SET_TITLE.emit(
                    "Chi Scan at R = {:3.2f} \u212B\u207B\u00B9".format(radius)
                )
                QtCore.QCoreApplication.processEvents()
                for nimg in range(self.startIndex, self.endIndex + 1):
                    if use_external_offset:
                        offset_x = int(
                            self.offset_df.at[nimg, "offset_x"] * scale_factor
                        )
                        offset_y = int(
                            self.offset_df.at[nimg, "offset_y"] * scale_factor
                        )
                        image_crop = [
                            1200 + VS + offset_y,
                            2650 + VS + offset_y,
                    ...
                    qImg, img = self.image_worker.get_image(
                        self._bit_depth,
                        image_list[nimg - self.startIndex],
                        autoWB,
                        brightness,
                        blackLevel,
                        image_crop,
                    )
                    RC, I = self.image_worker.get_chi_scan(
                        start,
                        radius * scale_factor,
                        width,
                        chiRange,
                    ...
                    if self.enableSync:
                        self.REFRESH_CANVAS.emit(
                            image_list[nimg - self.startIndex], image_crop
                        )
                    ...
                    map_2D360 = np.vstack(
                        (
                            map_2D360,
                            np.vstack((abs(RC), Phi360, I / normalization_factor)).T,
                        )
                    )
                    map_2D180 = np.vstack(
                        (map_2D180, np.vstack((RC, Phi180, I / normalization_factor)).T)
                    )
                    ...
                    self.PROGRESS_ADVANCE.emit(
                        0,
                        100,
                        (nimg + 1 - self.startIndex)
                        * 100
                        / (self.endIndex - self.startIndex + 1),
                    )
                    self.UPDATE_LOG.emit(
                        "The file being processed right now is: "
                        + image_list[nimg - self.startIndex]
                    )
                    QtCore.QCoreApplication.processEvents()
                    if self.enableSync:
            ...
            self.ERROR.emit(
                "The type of mapping is not consistent with the chosen region!"
            )
            QtCore.QCoreApplication.processEvents()
            ...
            if (
                ...
                if self.Is2D:
                    Kperp = (
                        np.abs(
                            (end.y() - start.y()) * origin.x()
                            - (end.x() - start.x()) * origin.y()
                            + end.x() * start.y()
                            - end.y() * start.x()
                        )
                        / np.sqrt(
                    ...
                    map_2D180 = np.array([0, 0, 0])
                    self.SET_TITLE.emit(
                        "Line Profile at Kperp = {:4.2f} (\u212B\u207B\u00B9)".format(
                            Kperp
                        )
                    )
                    QtCore.QCoreApplication.processEvents()
                    for nimg in range(self.startIndex, self.endIndex + 1):
                        ...
                        if width == 0.0:
                            RC, I = self.image_worker.get_line_scan(
                                start,
                                end,
                                img,
                                scale_factor,
                            ...
                            RC, I = self.image_worker.get_integral(
                                start,
                                end,
                                width,
                                img,
                        ...
                        if self.centralisationMethod == 1:
                            ...
                            if maxPos < (len(RC) - 1) / 2:
                                x360, y360 = (
                                    abs(RC[0 : (2 * maxPos + 1)] - RC[maxPos]),
                                    I[0 : (2 * maxPos + 1)] / normalization_factor,
                                )
                                map_2D360 = np.vstack(
                                    (
                                        map_2D360,
                                        np.vstack(
                                            (x360, Phi360[0 : (2 * maxPos + 1)], y360)
                                        ).T,
                                    )
                                )
                                x180, y180 = (
                                    RC[0 : (2 * maxPos + 1)] - RC[maxPos],
                                    I[0 : (2 * maxPos + 1)] / normalization_factor,
                                )
                                map_2D180 = np.vstack(
                                    (
                                ...
                                x360, y360 = (
                                    abs(
                                        RC[(2 * maxPos - len(RC) - 1) : -1] - RC[maxPos]
                                    ),
                                    I[(2 * maxPos - len(RC) - 1) : -1]
                            ...
                            map_2D360 = np.vstack(
                                (map_2D360, np.vstack((x360, Phi180, y360)).T)
                            )
                            x180, y180 = RC, I / normalization_factor
                            map_2D180 = np.vstack(
                                (map_2D180, np.vstack((x180, Phi180, y180)).T)
                            )
                            ...
                            map_2D360 = np.vstack(
                                (map_2D360, np.vstack((x360, Phi360, y360)).T)
                            )
                    ...
                    if not self._abort:
                        ...
                        map_2D_cart[:, 0] = map_2D_polar[:, 0] * np.cos(
                            (map_2D_polar[:, 1]) * Pi / 180
                        )
                        map_2D_cart[:, 1] = map_2D_polar[:, 0] * np.sin(
                            (map_2D_polar[:, 1]) * Pi / 180
                        )
                        self.graphTextPath = (
                            self.currentDestination
                        ...
                        if self.IsSaveResult:
                            if self.IsCartesian:
                                ...
                                np.savetxt(
                                    self.graphTextPath, map_2D_polar, fmt="%4.3f"
                                )
                        self.PROGRESS_END.emit()
                        self.UPDATE_LOG.emit("Completed!")
                    ...
                    for nimg in range(self.startIndex, self.endIndex + 1):
                        ...
                        for i in range(1, nos + 1):
                            ...
                            Kperp = (
                                np.abs(
                                    (newEnd.y() - newStart.y()) * origin.x()
                                    - (newEnd.x() - newStart.x()) * origin.y()
                                    + newEnd.x() * newStart.y()
                                    - newEnd.y() * newStart.x()
                                )
                                / np.sqrt(
                            ...
                            if width == 0.0:
                                ...
                                self.DRAW_RECT_REQUESTED.emit(
                                    newStart, newEnd, width, False
                                )
                            QtCore.QCoreApplication.processEvents()
                            ...
                            RC, I = RC.reshape(-1, self.group).mean(axis=1), I.reshape(
                                -1, self.group
                            ).mean(axis=1)
                            Phi360 = np.full(len(RC), nimg * 1.8)
                            Phi180 = np.full(len(RC), nimg * 1.8)
                            ...
                            if self.centralisationMethod == 1:
                                ...
                                K = np.full(len(x360), Kperp)
                                map_3D360 = np.vstack(
                                    (map_3D360, np.vstack((x360, Phi180, K, y360)).T)
                                )
                                x180, y180 = RC, I / normalization_factor
                                map_3D180 = np.vstack(
                                    (map_3D180, np.vstack((x180, Phi180, K, y180)).T)
                                )
                            elif self.centralisationMethod == 2:
                                for iphi in range(0, len(RC) // 2):
                                    Phi360[iphi] = nimg * 1.8 + 180
                                ...
                                K = np.full(len(x360), Kperp)
                                map_3D360 = np.vstack(
                                    (map_3D360, np.vstack((x360, Phi360, K, y360)).T)
                                )
                                x180, y180 = (
                                    RC - RC[len(RC) // 2],
                                    I / normalization_factor,
                            ...
                            self.PROGRESS_ADVANCE.emit(
                                0,
                                100,
                                (i + nos * (nimg - self.startIndex))
                                * 100
                                / ((self.endIndex - self.startIndex + 1) * nos),
                            )
                            self.UPDATE_LOG.emit(
                                "The file being processed right now is: "
                                + image_list[nimg - self.startIndex]
                    ...
                    if not self._abort:
                        ...
                        map_3D_cart[:, 0] = map_3D_polar[:, 0] * np.cos(
                            (map_3D_polar[:, 1]) * Pi / 180
                        )
                        map_3D_cart[:, 1] = map_3D_polar[:, 0] * np.sin(
                            (map_3D_polar[:, 1]) * Pi / 180
                        )
                        self.graphTextPath = (
                            self.currentDestination
                        ...
                        if self.IsSaveResult:
                            if self.IsCartesian:
                                ...
                                self.convertor_worker.txt2vtp(
                                    self.graphTextPath, "Cartesian"
                                )
                            else:
                                np.savetxt(
                                    self.graphTextPath, map_3D_polar, fmt="%4.3f"
                                )
                                self.convertor_worker.txt2vtp(
                                    self.graphTextPath, "Polar"
                                )
                        self.PROGRESS_END.emit()
                        self.UPDATE_LOG.emit("Completed!")
...
class DiffractionPattern(QtCore.QObject):
    ...
    def __init__(
        ...
        if form_factor_table is None:
            form_factor_table = peng_table
        if form_factor_table is None:
            raise ValueError("form_factor_table (or legacy peng_table) is required.")
        ...
        if self.Kx.shape[0] == 1 and self.Kx.shape[1] == 1:
            self.XYZ = np.dstack((self.Kx.T, self.Ky.T, self.Kz.T)).reshape(
                (self.Kx.shape[0], self.Kx.shape[1], self.Kx.shape[2], 3)
            )
        else:
            self.XYZ = np.dstack((self.Kx, self.Ky, self.Kz)).reshape(
                (self.Kx.shape[0], self.Kx.shape[1], self.Kx.shape[2], 3)
            )
        self.form_factor_table = form_factor_table
        self.peng_table = form_factor_table
    ...
    def atomic_form_factor(self, specie):
        warning = self.form_factor_table.consume_warning(specie)
        if warning:
            self.UPDATE_LOG.emit(warning)
            QtCore.QCoreApplication.processEvents()
        try:
            return self.form_factor_table.fe(self.Kx, self.Ky, self.Kz, specie)
        except KeyError as exc:
            self.ERROR.emit(str(exc))
            return np.zeros_like(self.Kx, dtype=float)
    ...
    def run(self):
        if self.useCUDA:
            ...
            try:
                ...
                self.UPDATE_LOG.emit(
                    "[ERROR] Unable to make context for this device. Process will be aborted."
                )
                self.ABORTED.emit()
            ...
            self.UPDATE_LOG.emit(
                ...
                + "({},{},{})".format(max_blkx, max_blky, max_blkz)
            )
            self.UPDATE_LOG.emit(
                "Maximum grid dimension: "
                + "({},{},{})".format(max_gridx, max_gridy, max_gridz)
            )
            ...
            self.UPDATE_LOG.emit(
                "Reciprocal space dimension: " + "({},{},{})".format(nkx, nky, nkz)
            )

            host_K_tensor = np.array(
                [
                    self.Kx * coord[0] + self.Ky * coord[1] + self.Kz * coord[2]
                    for coord in self.atoms_list.keys()
                ]
            ).astype(np.float32)
            host_atom_struct_fact = np.array(
                [self.af_dict[specie] for specie in self.atoms_list.values()]
            ).astype(np.float32)
            self.UPDATE_LOG.emit(
                "Total GPU memory (MB): "
                + "{:6.2f}".format(drv.mem_get_info()[1] / 1024 / 1024)
            )
            self.UPDATE_LOG.emit(
                "Available GPU memory (MB): "
                + "{:6.2f}".format(drv.mem_get_info()[0] / 1024 / 1024)
            )
            self.UPDATE_LOG.emit(
                "Requested GPU memory (MB): "
                + "{:6.2f}".format(host_atom_struct_fact.nbytes * 2 / 1024 / 1024)
            )

            if host_atom_struct_fact.nbytes * 2 > drv.mem_get_info()[0] * 0.95:
                self.UPDATE_LOG.emit(
                    "[ERROR] Insufficient GPU memory. Process will be aborted"
                )
                self.ABORTED.emit()
                self.INSUFFICIENT_MEMORY.emit()
                self._abort = False
                ...
                kernal(
                    device_atom_struct_fact,
                    device_K_tensor,
                    block=(nkz, 1, 1),
                    grid=(nb_atoms, nky, nkx),
                )
                end.record()
                end.synchronize()
            ...
            self.intensity = np.multiply(
                self.Psi.astype("complex64"), np.conj(self.Psi.astype("complex64"))
            )
...
class FitBroadening(QtCore.QObject):
    ...
    def run(self):
        ...
        for nimg in range(self.startIndex, self.endIndex + 1):
            ...
            self.UPDATE_LOG.emit(
                "The file being processed right now is: " + self.image_list[nimg]
            )
            QtCore.QCoreApplication.processEvents()
            qImg, img = self.image_worker.get_image(
                16,
                self.image_list[nimg],
                self.autoWB,
                self.brightness,
                self.blackLevel,
                self.image_crop,
            ...
            for i in range(1, nos + 1):
                ...
                if self.width == 0.0:
                    RC, I = self.image_worker.get_line_scan(
                        newStart, newEnd, img, self.scale_factor
                    )
                else:
                    RC, I = self.image_worker.get_integral(
                        newStart, newEnd, self.width, img, self.scale_factor
                    )
                if self.remove_linear_BGCheck:
                    BG = np.linspace(I[0], I[-1], len(I))
                    I = I - BG
                results, cost, output = self.fit_worker.get_fit(
                    RC,
                    I,
                    self.numberOfPeaks,
                    self.BGCheck,
                ...
                Kperp = (
                    np.abs(
                        (newEnd.y() - newStart.y()) * self.origin.x()
                        - (newEnd.x() - newStart.x()) * self.origin.y()
                        + newEnd.x() * newStart.y()
                        - newEnd.y() * newStart.x()
                    )
                    / np.sqrt(
                ...
                residual_variance = np.sum(results.fun**2) / (
                    len(I) - len(self.guess)
                )
                var = np.sqrt(np.diagonal(cov * residual_variance))
                value_variance = np.reshape(
                    np.concatenate((np.array(results.x), np.array(var)), axis=0),
                    (2, len(var)),
                )
                ...
                fitresults = (
                    str(angle).ljust(12)
                    + "\t"
                    + str(np.round(Kperp, 3)).ljust(12)
                    + "\t"
                    + "\t".join(
                        str(np.round(e[0], 3)).ljust(12)
                        + "\t"
                        + str(np.round(e[1], 3)).ljust(12)
                        for e in value_variance.T
                    )
                    + "\n"
                )
                ...
                if self._periodic:
                    ...
                    self.PROGRESS_ADVANCE.emit(
                        0,
                        100,
                        ((nimg + 101 * offset - self.startIndex) * nos + i)
                        * 100
                        / nos
                        / (self.endIndex - self.startIndex + 1),
                    )
...
class TAPD_Simulation(QtCore.QObject):
    ...
    def __init__(
        ...
        distribution,
        atom="S",
        in_plane_distribution="completely random",
        out_of_plane_distribution="completely random",
        buffer_offset=[0, 0, 0],
        sub_orientation="(001)",
        epi_orientation="(001)",
        use_atoms=True,
    ...
    def run(self):
        ...
        QtCore.QCoreApplication.processEvents()
        model = self.get_TAPD_sites(
            self.X_max,
            self.Y_max,
            self.Z_min,
            self.Z_max,
            self.offset,
            self.substrate_CIF_path,
            self.epilayer_CIF_path,
            self.buffer_layer_atom,
            self.buffer_in_plane_distribution,
            self.buffer_out_of_plane_distribution,
            self.buffer_offset,
            self.sub_orientation,
            self.epi_orientation,
            self.use_atoms,
            self.add_buffer,
        )
        QtCore.QCoreApplication.processEvents()
    ...
    def get_TAPD_sites(
        ...
        epilayer_CIF_path,
        atom,
        in_plane_distribution,
        out_of_plane_distribution,
        buffer_offset,
        sub_orientation="(001)",
        epi_orientation="(001)",
        use_atoms=True,
        ...
        model = TAPD_model()
        model.substrate_structure = load_ordered_structure(substrate_CIF_path)
        model.epilayer_structure = load_ordered_structure(epilayer_CIF_path)
        self.UPDATE_LOG.emit("Preparing the substrate ...")
        QtCore.QCoreApplication.processEvents()
        substrate_set, model.substrate_list, model.substrate_sites = self.get_substrate(
            model.substrate_structure,
            sub_orientation,
            X_max,
            Y_max,
            Z_min,
        ...
        if not substrate_set is None:
            ...
            QtCore.QCoreApplication.processEvents()
            generators = self.generator_2D(
                substrate_set, self.distribution, **self.parameters
            )
            if isinstance(generators, int):
                (
                    model.epilayer_list,
                    model.epilayer_sites,
                    model.epilayer_domain_area_list,
                    model.epilayer_domain_boundary_list,
                    model.epilayer_boundary_sites,
                    model.epilayer_domain,
                ) = self.get_boundaryless_epilayer(
                    model.epilayer_structure,
                    epi_orientation,
                    X_max,
                    Y_max,
                    Z_min,
        ...
        if add_buffer:
            ...
            QtCore.QCoreApplication.processEvents()
            model.buffer_layer_list, model.buffer_layer_sites = self.get_buffer_layer(
                model.epilayer_list,
                max(
                    model.epilayer_structure.lattice.a,
                    model.epilayer_structure.lattice.b,
                ),
                atom,
                in_plane_distribution,
                out_of_plane_distribution,
            ...
            while substrate_set:
                ...
                if i <= number_of_sites:
                    ...
                    self.PROGRESS_ADVANCE.emit(
                        ...
                        100,
                        np.round(
                            100 - len(substrate_set) / substrate_set_length * 100, 1
                        ),
                    )
                    QtCore.QCoreApplication.processEvents()
                else:
    ...
    def get_buffer_layer(
        ...
        **kwargs
        ...
        while epilayer_set:
            ...
            x = x0 + 0.1 * (
                substrate_lattice_constant * np.random.random_sample()
                - substrate_lattice_constant / 2
            )
            y = y0 + 0.1 * (
                substrate_lattice_constant * np.random.random_sample()
                - substrate_lattice_constant / 2
            ...
            if out_of_plane_distribution == "gaussian":
                z = np.random.normal(
                    (kwargs["z_low"] + kwargs["z_high"]) / 2,
                    kwargs["z_high"] - kwargs["z_low"],
                )
                if z < kwargs["z_low"]:
                    z = kwargs["z_low"]
                if z > kwargs["z_high"]:
                ...
                z = (
                    np.random.random_sample() * (kwargs["z_high"] - kwargs["z_low"])
                    + kwargs["z_low"]
                )
            rand_atom_list.append([x + offset[0], y + offset[1]])
            rand_atom_sites.append(
                pgSites.Site(atom, [x + offset[0], y + offset[1], z + offset[2]])
            )
            if in_plane_distribution == "completely random":
                ...
                if in_plane_distribution == "geometric":
                    ...
                    radius = np.random.uniform(
                        kwargs["buffer_in_plane_low"], kwargs["buffer_in_plane_high"]
                    )
            ...
            if i <= number_of_sites:
                ...
                self.PROGRESS_ADVANCE.emit(
                    0,
                    100,
                    np.round(100 - len(epilayer_set) / epilayer_set_length * 100, 1),
                )
                QtCore.QCoreApplication.processEvents()
            else:
                break
    ...
    def get_substrate(
        self, structure, orientation, X_max, Y_max, Z_min, Z_max, use_atoms
    ):
        if use_atoms:
            unit_cell_sites_sub = [
                pgSites.Site(
                    {
                        site.as_dict()["species"][0]["element"]: site.as_dict()[
                            "species"
                        ][0]["occu"]
                    },
                ...
                for site in structure.sites
                if (
                    site.z >= Z_min * structure.lattice.c
                    and site.z < Z_max * structure.lattice.c
                )
            ]
        else:
        ...
        for i, j in itertools.product(range(-xa, xa + 1), range(-yb, yb + 1)):
            ...
            if self._abort:
                ...
                self.PROGRESS_ADVANCE.emit(
                    0, 100, ((i + xa) * (2 * yb) + (j + yb)) / (2 * xa * 2 * yb) * 100
                )
                QtCore.QCoreApplication.processEvents()
        self.PROGRESS_END.emit()
        QtCore.QCoreApplication.processEvents()
    ...
    def get_boundaryless_epilayer(
        self, structure, orientation, X_max, Y_max, Z_min, Z_max, offset, use_atoms
    ):
        if use_atoms:
            unit_cell_sites_epi = [
                pgSites.Site(
                    ...
                    [site.x + offset[0], site.y + offset[1], site.z + offset[2]],
                )
                for site in structure.sites
                if (
                    site.z >= Z_min * structure.lattice.c
            ...
            species = self.get_heavist_element(structure)
            unit_cell_sites_epi = [
                pgSites.Site({species: 1}, [offset[0], offset[1], offset[2]])
            ]
        ...
        (
            epilayer_list,
            epilayer_sites,
            epilayer_domain_area,
            epilayer_domain_boundary,
            gap_add_out,
            gap_subtract_out,
            domain,
        ) = self.get_boundaryless_domain(a, b, angle, rectangle, unit_cell_sites_epi)
        return (
            epilayer_list,
            epilayer_sites,
            [epilayer_domain_area],
    ...
    def get_epilayer(
        self, vor, structure, orientation, X_max, Y_max, Z_min, Z_max, offset, use_atoms
    ):
        epilayer_list = []
        epilayer_sites = []
        ...
        for point_index, region_index in enumerate(vor.point_region):
            self.PROGRESS_ADVANCE.emit(
                0, 100, np.round(point_index / len(vor.point_region) * 100, 1)
            )
            QtCore.QCoreApplication.processEvents()
                ...
                if original_polygon.is_valid:
                    ...
                    if polygon:
                        (
                            ...
                            epilayer_domain_area,
                            epilayer_domain_boundary,
                            epilayer_domain_boundary_sites,
                            gap_add_out,
                            gap_subtract_out,
                            domain,
                        ) = self.get_domain(
                        ...
                        if epilayer_domain:
                            ...
                            if not -1 in vor.regions[region_index]:
                                ...
                                epilayer_domain_boundary_list.append(
                                    epilayer_domain_boundary
                                )
                                epilayer_boundary_sites += (
                                    epilayer_domain_boundary_sites
    ...
    def get_region_vertices_dict(self, vor, x_max, y_max):
        ...
        rectangle = Polygon(
            [
                (-oversize_factor * x_max, -oversize_factor * y_max),
                (-oversize_factor * x_max, oversize_factor * y_max),
                (oversize_factor * x_max, oversize_factor * y_max),
                (oversize_factor * x_max, -oversize_factor * y_max),
            ]
        ...
        for pointidx, simplex in zip(vor.ridge_points, vor.ridge_vertices):
            ...
            if np.any(simplex < 0):
                ...
                intersection = rectangle.intersection(
                    LineString(
                        [
                            (vor.vertices[i, 0], vor.vertices[i, 1]),
                            (far_point[0], far_point[1]),
                        ]
                    )
                )
            if intersection:
                region_vertices_dict[tuple(vor.points[pointidx[0]])].append(
                    list(intersection.coords[0])
                )
                region_vertices_dict[tuple(vor.points[pointidx[0]])].append(
                    list(intersection.coords[1])
                )
                region_vertices_dict[tuple(vor.points[pointidx[1]])].append(
                    list(intersection.coords[0])
                )
                region_vertices_dict[tuple(vor.points[pointidx[1]])].append(
                    list(intersection.coords[1])
    ...
    def point_in_triangle(self, v1, v2, v3, p, r):
        ...
        return (
            ...
            or self.within_vertex(v1, p, r)
            or self.within_vertex(v2, p, r)
            or self.within_vertex(v3, p, r)
        )
    ...
    def get_boundaryless_domain(self, a, b, angle, rectangle, unit_cell_sites_epi):
        ...
        for i, j in itertools.product(
            range(int(bounding_box[0] / a) - 5, int(bounding_box[2] / a) + 6),
            range(
                int(bounding_box[1] / b / np.sin(angle / 180 * np.pi)) - 5,
                int(bounding_box[3] / b / np.sin(angle / 180 * np.pi)) + 6,
            ),
        ):
            offset = int(j * b * np.cos(angle / 180 * np.pi) / a)
            x = (i - offset) * a + j * b * np.cos(angle / 180 * np.pi)
    ...
    def get_domain(
        ...
        if (domain_include.is_valid) or (
            np.all(subdomain.is_valid for subdomain in domain_include)
        ):
            epilayer_domain = []
            epilayer_domain_sites = []
            hull_sites = []
            ...
            for i, j in itertools.product(
                ...
                try:
                    condition1 = domain_include.buffer(max(a, b) * 0.001).intersects(
                        Point(x, y)
                    )
                except:
                    condition1 = np.any(
                        subdomain.buffer(max(a, b) * 0.001).intersects(Point(x, y))
                        for subdomain in domain_include
                    )
                try:
                    condition2 = not domain_exclude.contains(Point(x, y))
                except:
                    condition2 = not np.any(
                        subdomain.contains(Point(x, y)) for subdomain in domain_exclude
                    )
                if condition1 and condition2:
                    ...
                    for site in unit_cell_sites_epi:
                        epilayer_domain_sites.append(
                            pgSites.Site(
                                {
                                    site.as_dict()["species"][0][
                                        "element"
                                    ]: site.as_dict()["species"][0]["occu"]
                                },
                                [site.x + x, site.y + y, site.z],
                            )
                        )
            ...
            try:
                ...
                for vertex_index in hull.coplanar:
                    if not (
                        hull.points[vertex_index[0]][0] in x_list
                        and hull.points[vertex_index[0]][1] in y_list
                    ):
                    ...
                    for site in unit_cell_sites_epi:
                        hull_sites.append(
                            pgSites.Site(
                                ...
                                [site.x + point[0], site.y + point[1], site.z],
                            )
                        )
            ...
            if point_cloud.is_valid:
                for first, second in zip(vertices, vertices[1:]):
                    gap_add = Polygon([first, second, point]).difference(
                        point_cloud.buffer(max(a, b) * 0.999, resolution=64)
                    )
                    gap_subtract = point_cloud.buffer(
                        max(a, b) * 0.999, resolution=64
                    ).difference(Polygon([first, second, point]))
