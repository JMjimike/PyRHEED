...
class Window:
    ...
    def __init__(self):
        ...
        config.read(os.path.join(self.dirname, "configuration.ini"))
        self.app = QtWidgets.QApplication(sys.argv)
        # 192//(self.app.primaryScreen().logicalDotsPerInch())
        ...
        self.app.setFont(
            QtGui.QFont(
                ...
                int(self.app.font().pointSize() * self.screenScaleFactor),
            )
        )
        ...
        self.darkPalette.setColor(
            QtGui.QPalette.ColorRole.Window, QtGui.QColor(53, 53, 53)
        )
        self.darkPalette.setColor(
            QtGui.QPalette.ColorRole.WindowText, QtCore.Qt.GlobalColor.white
        )
        self.darkPalette.setColor(
            QtGui.QPalette.ColorRole.Base, QtGui.QColor(25, 25, 25)
        )
        self.darkPalette.setColor(
        ...
        self.darkPalette.setColor(
            QtGui.QPalette.ColorRole.Button, QtGui.QColor(53, 53, 53)
        )
        self.darkPalette.setColor(
    ...
    def toggle_light_dark_mode(self, mode):
        ...
        if mode == "light":
            ...
            for i in range(0, self.window.mainTab.count()):
                self.window.mainTab.widget(i).setBackgroundBrush(
                    QtGui.QBrush(QtGui.QColor("darkGray"))
                )
        elif mode == "dark":
            self.app.setPalette(self.darkPalette)
            for i in range(0, self.window.mainTab.count()):
                self.window.mainTab.widget(i).setBackgroundBrush(
                    QtGui.QBrush(QtGui.QColor(50, 50, 50))
                )
        self.app.setStyle("fusion")
    ...
    def run_scenario(self):
        self.scenario_window = scenario.Window(self.appTheme)
    ...
    def run_reciprocal_space_mapping(self, path):
        ...
        self.reciprocal_space_mapping.CONNECT_TO_CANVAS.connect(
            self.connect_mapping_to_canvas
        )
        self.window.RETURN_STATUS.connect(self.reciprocal_space_mapping.set_status)
        self.preference.DEFAULT_SETTINGS_CHANGED.connect(
            self.reciprocal_space_mapping.refresh
        )
    ...
    def run_2D_contour(
        ...
        """
        self.graph = graph_3D_surface.Graph()
        self.graph.show_2d_contour(
            path,
            insideGraph3D=insideGraph3D,
            min=min,
            max=max,
    ...
    def connect_gmm_to_canvas(self):
        """Signal Connection"""
        self.gmm.DRAW_LINE_REQUESTED.connect(
            self.window.mainTab.currentWidget().draw_line
        )
        self.gmm.DRAW_RECT_REQUESTED.connect(
            self.window.mainTab.currentWidget().draw_rect
        )

    def connect_broadening_to_canvas(self):
        """Signal Connection"""
        self.broadening.DRAW_LINE_REQUESTED.connect(
            self.window.mainTab.currentWidget().draw_line
        )
        self.broadening.DRAW_RECT_REQUESTED.connect(
            self.window.mainTab.currentWidget().draw_rect
        )

    def connect_mapping_to_canvas(self):
        """Signal Connection"""
        self.reciprocal_space_mapping.DRAW_LINE_REQUESTED.connect(
            self.window.mainTab.currentWidget().draw_line
        )
        self.reciprocal_space_mapping.DRAW_RECT_REQUESTED.connect(
            self.window.mainTab.currentWidget().draw_rect
        )
