...
class Window(QtCore.QObject):
    ...
    REFRESH_POLAR_I = QtCore.pyqtSignal(
        np.ndarray, np.ndarray, str, str, int, str, bool, dict
    )
    REFRESH_POLAR_F = QtCore.pyqtSignal(
        np.ndarray, np.ndarray, str, str, int, str, bool, dict
    )
    NORMAL_I_REQUESTED = QtCore.pyqtSignal()
    NORMAL_F_REQUESTED = QtCore.pyqtSignal()
    REFRESH_NORMAL_I = QtCore.pyqtSignal(
        np.ndarray, np.ndarray, str, str, int, str, bool, dict
    )
    REFRESH_NORMAL_F = QtCore.pyqtSignal(
        np.ndarray, np.ndarray, str, str, int, str, bool, dict
    )
    ...
    def main(self, path, preload=False):
        ...
        self.chooseSourceLabel = QtWidgets.QLabel(
            "The path of the report file is:\n" + self.path
        )
        self.chooseSourceButton = QtWidgets.QPushButton("Browse...")
        self.chooseSourceButton.setSizePolicy(
        ...
        self.ReportInformation = QtWidgets.QLabel(
        ...
        self.figureGeneratorLabel = QtWidgets.QLabel(
            "Choose the type of figure generator:"
        )
        ...
        self.KperpLabel = QtWidgets.QLabel(
            "Kperp = {:6.2f} (\u212B\u207B\u00B9)".format(
                self.currentKP / self.KperpSliderScale + self.RangeStart
            )
        )
        ...
        self.AzimuthLabel = QtWidgets.QLabel(
            "Azimuth Angle = {:5.1f} (\u00B0)".format(self.currentAzimuth)
        )
        ...
        self.intensityRangeSlider = DoubleSlider(
            minimum=0,
            maximum=200,
            scale=0.01,
            head=0,
        ...
        self.HWHMRangeSlider = DoubleSlider(
            minimum=0,
            maximum=200,
            scale=0.01,
            head=0,
        ...
        self.ButtonBox.addButton(
            "Fit", QtWidgets.QDialogButtonBox.ButtonRole.AcceptRole
        )
        self.ButtonBox.addButton(
            "Quit", QtWidgets.QDialogButtonBox.ButtonRole.DestructiveRole
        )
        self.ButtonBox.setCenterButtons(True)
        self.ButtonBox.findChildren(QtWidgets.QPushButton)[1].clicked.connect(
            self.linear_fit
        )
        self.ButtonBox.findChildren(QtWidgets.QPushButton)[2].clicked.connect(
            self.reject
        )
        ...
        self.Dialog.show()
        desktopRect = self.Dialog.screen().availableGeometry()
        center = desktopRect.center()
        self.Dialog.move(
            center.x() - self.Dialog.width() * 0.5,
            center.y() - self.Dialog.height() * 0.5,
        )
    ...
    def connect_OK_button(self, text):
        ...
        if text == "Qt":
            self.ButtonBox.findChildren(QtWidgets.QPushButton)[0].clicked.connect(
                self.polar_start
            )
    ...
    def choose_source(self):
        path = QtWidgets.QFileDialog.getOpenFileName(
            None, "Choose The Report File", self.path
        )
        self.path = path[0]
        ...
        if not self.path == "":
            if not self.pathExtension == ".txt":
                ...
                self.chooseSourceLabel.setText(
    ...
    def start(self):
        ...
        if self.IA.checkState() == 2:
            ...
            self.plot_IA(
                ...
                A,
                Kp,
                self.intensityRangeSlider.values()[0],
                self.intensityRangeSlider.values()[1],
            )
        if self.FA.checkState() == 2:
            ...
            self.plot_FA(
                ...
                A,
                Kp,
                self.HWHMRangeSlider.values()[0],
                self.HWHMRangeSlider.values()[1],
            )
        plt.show()

    def toggle_dark_mode(self, mode):
        self.TOGGLE_DARK_MODE_REQUEST.emit(mode)
        self.REFRESH_NORMAL_I = QtCore.pyqtSignal(
            np.ndarray, np.ndarray, str, str, int, str, bool, dict
        )
        self.REFRESH_NORMAL_F = QtCore.pyqtSignal(
            np.ndarray, np.ndarray, str, str, int, str, bool, dict
        )
    ...
    def polar_start(self):
        ...
        if self.IA.checkState() == 2:
            ...
            self.REFRESH_POLAR_I.emit(
                I,
                A,
                "IA",
                self.fontList.currentFont().family(),
                self.fontSizeSlider.value(),
                self.IAColor.get_color(),
                False,
                {
                    "Kp": Kp,
                    "low": self.intensityRangeSlider.values()[0],
        ...
        if self.IK.checkState() == 2:
            ...
            self.IKIsPresent = True
            self.REFRESH_NORMAL_I.emit(
                K,
                I,
                "IK",
                self.fontList.currentFont().family(),
                self.fontSizeSlider.value(),
                self.IKColor.get_color(),
                True,
                {"Az": Az},
            )
        if self.FA.checkState() == 2:
            ...
            self.REFRESH_POLAR_F.emit(
                F,
                A,
                "FA",
                self.fontList.currentFont().family(),
                self.fontSizeSlider.value(),
                self.FAColor.get_color(),
                False,
                {
                    "Kp": Kp,
                    "low": self.HWHMRangeSlider.values()[0],
        ...
        if self.FK.checkState() == 2:
            ...
            self.FKIsPresent = True
            self.REFRESH_NORMAL_F.emit(
                K,
                F,
                "FK",
                self.fontList.currentFont().family(),
                self.fontSizeSlider.value(),
                self.FKColor.get_color(),
                True,
                {"Az": Az},
            )
    ...
    def plot_IA(self, I, A, Kp, imin, imax):
        ...
        ax.set_title(
            "Intensity vs Azimuth at Kperp = {:5.2f} (\u212B\u207B\u00B9)".format(Kp),
    ...
    def plot_IK(self, I, K, Az):
        ...
        ax = plt.subplot()
        ax.set_title(
            "Intensity vs Kperp at Phi = {:5.2f}\u00B0".format(Az),
            fontsize=self.fontSizeSlider.value(),
        )
    ...
    def plot_FA(self, F, A, Kp, fmin, fmax):
        ...
        ax = plt.subplot(projection="polar")
        ax.set_title(
            "HWHM vs Azimuth at Kperp = {:5.2f} (\u212B\u207B\u00B9)".format(Kp),
            fontsize=self.fontSizeSlider.value(),
        )
    ...
    def plot_FK(self, F, K, Az):
        ...
        ax = plt.subplot()
        ax.set_title(
            "HWHM vs Kperp at Phi = {:5.2f}\u00B0".format(Az),
            fontsize=self.fontSizeSlider.value(),
        )
    ...
    def plot_IFK(self, I, F, K, Az):
        ...
        fig.suptitle(
            "Intensity and HWHM vs Kperp at Phi = {:5.2f}\u00B0".format(Az),
            fontsize=self.fontSizeSlider.value(),
        )
        ax1.set_xlabel(
            "Intensity (arb. units)",
            fontsize=self.fontSizeSlider.value(),
            color=self.IKColor.get_color(),
        )
    ...
    def get_IA(self):
        ...
        rows = np.full(self.AZmax + 1, self.currentKP).astype(int) + np.linspace(
            0, self.AZmax, self.AZmax + 1
        ).astype(int) * (self.KPmax + 1)
        A = self.Angles
        I = np.fromiter((self.report[i, columnI] for i in rows.tolist()), float)
        Ierror = np.fromiter(
            (self.report[i, columnIerror] for i in rows.tolist()), float
        )
        return I, A, Ierror
    ...
    def get_FA(self):
        ...
        if self.fit_function == "Gaussian":
            ...
            Ferror = np.fromiter(
                (self.report[i, columnFerror] / 2 for i in rows.tolist()), float
            )
        elif self.fit_function == "Voigt":
            F = np.fromiter(
                (
                    (
                        0.5346 * self.report[i, columnFL]
                        + np.sqrt(
                            0.2166 * self.report[i, columnFL] ** 2
                            + self.report[i, columnFG] ** 2
                        )
                    )
                    / 2
            ...
            Ferror = np.fromiter(
                (
                    (
                        0.5346 * self.report[i, columnFLerror]
                        + (
                            0.4332 * self.report[i, columnFLerror]
                            + 2 * self.report[i, columnFGerror]
                        )
                        / np.sqrt(
                            0.2166 * self.report[i, columnFL] ** 2
                            + self.report[i, columnFG] ** 2
    ...
    def get_IK(self):
        ...
        if self.fit_function == "Gaussian":
            ...
            columnI = int(1 + nop * 2 + peakIndex * 2 + 1)
            columnIerror = int(1 + nop * 2 + peakIndex * 2 + 2)
        rows = np.full(self.KPmax + 1, self.currentAzimuth * (self.KPmax + 1)).astype(
            int
        ) + np.linspace(0, self.KPmax, self.KPmax + 1).astype(int)
        K = self.Kperps
        I = np.fromiter((self.report[i, columnI] for i in rows.tolist()), float)
        Ierror = np.fromiter(
            (self.report[i, columnIerror] for i in rows.tolist()), float
    ...
    def linear_fit(self):
        file_name = QtWidgets.QFileDialog.getSaveFileName(
            ...
            os.path.join(self.dirname, "linear_fit_results.txt"),
            "TXT (*.txt)",
        )
        if file_name:
    ...
    def load_report(self, path):
        ...
        self.startIndex = self.header.get(
            "StartImageIndex", int(self.AzimuthStart / 1.8)
        )
        ...
        self.AzimuthLabel.setText(
            "Azimuth Angle = {:5.1f} (\u00B0)".format(
                self.currentAzimuth * 1.8 + self.AzimuthStart
            )
        )
        self.AzimuthSlider.setMaximum(self.AZmax)
        self.AzimuthSlider.setValue(0)
        self.KperpLabel.setText(
            "Kperp = {:6.2f} (\u212B\u207B\u00B9)".format(
                self.currentKP / self.KperpSliderScale + self.RangeStart
            )
        )
        if self.BGCheck:
            if len(self.Kperps) > 1:
                self.ReportInformation.setText(
                    "Date of the report: "
                    + self.date
                    + "Fit function: "
                    + self.fit_function
                    + "\n"
                    + "Number of peaks: {}\nStart image index: {}\nEnd image index: {}\nStart Kperp position: {:5.2f} (\u212B\u207B\u00B9)\nEnd Kperp position: {:5.2f} (\u212B\u207B\u00B9)\nKperp step size: {:5.2f} (\u212B\u207B\u00B9)".format(
                        self.NumberOfPeaks + 1,
                        self.startIndex,
                        self.endIndex,
                        self.RangeStart,
                        self.RangeEnd,
                        (self.Kperps[1] - self.Kperps[0]),
                    )
                )
            else:
                self.ReportInformation.setText(
        ...
        if self.BGCheck:
            ...
            for i in range(
                5 - int((self.NumberOfPeaks - 1) / 2),
                5 + int((self.NumberOfPeaks - 1) / 2) + 1,
            ):
                self.peak.addItem(
                    peaks[i], str(i - int((5 - (self.NumberOfPeaks - 1) / 2)))
                )
    ...
    def check_start_OK(self):
        if (
            self.IA.checkState() == 0
            and self.IK.checkState() == 0
            and self.FA.checkState() == 0
            and self.FK.checkState() == 0
        ):
