...
class PlotChart(QtWidgets.QWidget):
    ...
    def update_coordinate(self, pos):
        if self.type == "Polar":
            self.coordinate.setText(
                "(r={:5.2f}, \u03C6={:5.2f})".format(pos.y(), pos.x())
            )
        elif self.type == "Normal":
    ...
    def add_chart(
        self, radius, profile, preset, fontname, fontsize, color, switchXY, kwargs
    ):
        ...
        if self.type == "Polar":
            ...
            if preset == "IA":
                ...
                self.profileChart.setTitle(
                    "Intensity vs Azimuth at Kperp = {:6.2f} (\u212B\u207B\u00B9)".format(
                        z
                    )
                )
                ...
                high = kwargs["high"]
                self.profileChart.setTitle(
                    "HWHM vs Azimuth at Kperp = {:6.2f} (\u212B\u207B\u00B9)".format(z)
                )
            ...
            self.profileChart.addAxis(
                self.axisR, QtCharts.QPolarChart.PolarOrientationRadial
            )
            self.profileChart.addAxis(
                self.axisP, QtCharts.QPolarChart.PolarOrientationAngular
            )
            ...
            self.chartView.setChart(self.profileChart)
            self.chartView.setContextMenuPolicy(
            ...
            if preset == "IK":
                Az = kwargs["Az"]
                self.profileChart.setTitle(
                    "Intensity vs Kperp at \u03C6 = {:5.1f} (\u00BA)".format(Az)
                )
    ...
    def save_polar_as_text(self):
        if self.CHART_IS_PRESENT:
            self.filename = QtWidgets.QFileDialog.getSaveFileName(
                ...
                os.join(self.dirname, "polar.txt"),
                "Text (*.txt)",
            )
